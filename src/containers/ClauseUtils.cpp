 ...
 */
static inline bool
areClausesEqual(const lit_t* left,
                const csize_t leftSize,
                const lit_t* right,
                const csize_t rightSize)
{
  ...
  for (uint li = 0; li < leftSize; li++) {
    ...
    // Same elements (if sorted and equal will only continue => linear
    // complexity)
    if (left[li] == right[ri]) {
      ri++;
      continue;
...
bool
ClauseEqual::operator()(const clause_t& left, const clause_t& right) const
{
  return areClausesEqual(left.data(), left.size(), right.data(), right.size());
}

bool
CLikeClauseEqual::operator()(const ClikeClause& left,
                             const ClikeClause& right) const
{
  return areClausesEqual(left.lits, left.size, right.lits, right.size);
}

bool
ClauseExchangeEqual::operator()(const ClauseExchange& left,
...
bool
ClauseExchangePtrEqual::operator()(const ClauseExchangePtr& left,
...
hash_t
ClauseHash::operator()(const clause_t& clause) const
{
...
getLiteralsCount(const std::vector<ClauseExchangePtr>& clauses)
{
  return std::accumulate(
    clauses.begin(), clauses.end(), 0, [](int sum, const auto& clause) {
