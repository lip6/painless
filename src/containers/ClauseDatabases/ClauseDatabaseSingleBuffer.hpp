 ...
 *
 * I wanted ClauseBuffer to not implement ClauseDatabase to make it easier to
 * change(independent).
 *
 */
...
public:
  ...
  {
    return buffer.addClause(std::move(clause));
  }
   ...
   *
   * @note This method fills the buffer without exceeding  by getting one clause
   * at a time.
   * @warning This method may not select exactly  literals if the last clause
   * added exceeds the limit.
   */
   ...
   *
   * @warning This method clears the entire database. All clauses will be
   * removed.
   */
  /// Does nothing
