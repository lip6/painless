 ...
 * @brief A clause database implementation attempting to naively mimic the
 * Simplified Adaptive Database from Mallob
 * (ref:https://doi.org/10.1613/jair.1.15827).
 *
 * This class implements a clause database that organizes clauses based on their
 * size and LBD (Literal Block Distance). It uses a vector of ClauseBuffers to
 * store clauses, with each buffer corresponding to a specific size and LBD
 * range. The implementation aims to provide efficient clause storage,
 * retrieval, and management while maintaining a balance between clause quality
 * and database size.
 *
 * Key features:
 * - Clauses are partitioned based on size and LBD.
 * - Supports concurrent additions with lock-free mechanisms for unit clauses.
 * - Implements a shrinking mechanism to maintain the database size within
 * capacity.
 *
...
public:
   ...
   * @param maxFreeSize Maximum size for which giveSelection does not decrement
   * the literal count while filling the export buffer.
   * @throw std::invalid_argument if any parameter is invalid.
   */
  explicit ClauseDatabaseMallob(int maxClauseSize,
   ...
   *
   * This method attempts to add a clause to the appropriate buffer based on its
   * size and LBD. It uses shared locks for synchronizing the
   * m_currentLiteralCounts and the other class atributes with shrinkDatabase.
   * Units are always added, capacity is ignored for them.
   *
   ...
   *
   * This method selects clauses from the database, prioritizing unit clauses
   * and then iterating through the clause buffers to fill the selection up to
   * the specified limit.
   *
  ...
  /**
   * @brief Retrieves a single clause from the best (smallest index) buffer
   * possible.
   *
   * This method attempts to retrieve a clause from any non-empty buffer in the
   * database.
   *
  ...
  /**
   * @brief Shrinks the database by removing clauses to maintain the size within
   * capacity.
   *
   * This method removes clauses from the worst (highest index) buffers until
   * the database size is within the specified capacity. However it starts by
   * adding missed clauses addition due to the previous shrink unique_lock. Unit
...
protected:
   ...
   *
   * This function computes an index in the clauses vector based on the clause's
   * size and LBD. It uses m_maxPartitioningLbd to determine the partitioning of
   * LBD values.
   *
   ...
   * @throw std::invalid_argument if size is less than UNIT_SIZE or lbd is less
   * than MIN_LBD.
   *
   * Index calculation:
   * - For each size, there are m_maxPartitioningLbd possible indices.
   * - LBD values >= (MIN_LBD + m_maxPartitioningLbd) are grouped together in
   * the last partition.
   *
  ...
  inline unsigned getIndex(int size, int lbd) const
  {
    ...
    return (size - 1) * m_maxPartitioningLbd +
           std::min(lbd - MIN_LBD, m_maxPartitioningLbd - 1);
  }
   ...
   */
  inline int getSizeFromIndex(unsigned index) const
  {
    return (index == 0) ? 1 : (index / m_maxPartitioningLbd) + 1;
  }
   ...
   */
  inline int getLbdPartitionFromIndex(unsigned index) const
  {
    return (index % m_maxPartitioningLbd) + MIN_LBD;
  }

  size_t
    m_totalLiteralCapacity; ///< Maximum total literal capacity of the database.
  int m_maxPartitioningLbd; ///< Maximum LBD value for separate partitioning.
  int m_maxClauseSize;      ///< Maximum size of clauses to be stored.
  int m_freeMaxSize;        ///< Maximum size for which giveSelection does not
                            ///< count in while filling exportBuffer.

  std::vector<std::unique_ptr<ClauseBuffer>>
    m_clauses; ///< Vector of clause buffers, indexed by size and LBD.
  std::atomic<long>
    m_currentLiteralSize; ///< Current number of literals in the database
                          ///< (excluding unit clauses).
  std::atomic<int>
    m_currentWorstIndex; ///< Index of the current worst clause in the database.
  mutable std::shared_mutex
    m_shrinkMutex; ///< Mutex used to coordinate shrinking and clause addition.
