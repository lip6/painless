 ...
 * @brief A clause database that maintain separate buffers for each entity,
 * mimicing old painless default behavior
 *
 * This class extends ClauseDatabase to provide a mechanism for storing and
 * managing clauses in separate buffers for different entities thanks to the
 * from attributed in ClauseExchange. It uses fine-grained locking to allow
 * concurrent access where possible.
 *
 * Since ClauseBuffer is lockfree, all consumption operations (read) can be done
 * concurently. The only synchronization required is on the map of buffers
 *
 * The maximum size parameter is only used at selection, in the temporary
 * ClauseDatabasePerSize instance.
 *
...
public:
   ...
   * @note This method is thread-safe and allows concurrent additions to
   * different buffers.
   */
   ...
   * @note This method acquires a shared lock and can be called concurrently
   ...
   * @note This method acquires an exclusive lock and cannot be called
...
private:
   ...
   */
  mutable std::shared_mutex m_databaseMX;
