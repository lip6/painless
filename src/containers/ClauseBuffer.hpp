 ...
 *
 * This class utilizes a non-fixed size boost::lockfree::queue for efficient,
 * lock-free operations between multiple producers and consumers. It uses raw
 * pointers internally and provides a thread-safe interface for
 * ClauseExchangePtr.
...
{
private:
  boost::lockfree::queue<ClauseExchange*, boost::lockfree::fixed_sized<false>>
    queue;
  std::atomic<size_t> m_size; ///< Tracks the number of elements in the queue

public:
  static constexpr const unsigned long DEFAULT_SIZE = 1'000;
  ...
  bool addClause(ClauseExchangePtr clause)
  {
    ...
    if (queue.push(raw)) {
      ...
      // Reference count is decremented, since we do not store the returned
      // ClauseExchangePtr, thus it goes out of scope
   ...
   *
   * This method tries to add a clause to the buffer using Boost's bounded_push
   * operation. If the internal memory pool of the queue is exhausted, the
   * operation will fail.
   *
   * @param clause The clause to add to the buffer.
   * @return true if the clause was successfully added, false if the buffer is
   * full or push failed.
   */
   ...
   *
   * This method tries to add clauses to the buffer until either all clauses are
   * added or a push operation fails (indicating the buffer is full).
   *
  ...
  {
    LOGD4("Size before pop %ld", this->size());
    ClauseExchange* raw;
    if (queue.pop(raw)) {
