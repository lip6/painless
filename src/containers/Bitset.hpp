...
public:
   ...
   * @details This constructor initializes a Bitset with the given number of
   * bits. All bits are set to the specified default value. If the size is not a
   * multiple of BITS_PER_BLOCK (typically 64), the constructor ensures that any
   * unused bits in the last block are set to zero, maintaining consistency of
   * the bitset.
   */
    ...
    : num_bits(size)
  {
    ...
    if (num_bits % BITS_PER_BLOCK != 0) {
      //                   |       out of bound       |
      // (1ULL << 40) - 1 = 00000000 00000000 00000000 11111111 11111111
  ...
  /**
   * @brief Merge multiple bitsets with this bitset using a custom binary
   * operation.
   *
   * @tparam BinaryOp A callable type that takes two unsigned long long
   * arguments and returns an unsigned long long. This type is deduced from the
   ...
   *           (function, lambda, or function object) that takes two unsigned
   * long long arguments and returns an unsigned long long.
   *
   ...
   *     bitset.merge(other_bitsets, [](unsigned long long a, unsigned long long
   * b) { return a | b; });
   *
   ...
   *     unsigned long long custom_or(unsigned long long a, unsigned long long
   * b) { return a | b; } bitset.merge(other_bitsets, custom_or);
   */
  ...
  {
    merge(other_bitsets, std::bit_or<unsigned long long>());
  }
