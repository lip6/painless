...
activate_literal(kissat *solver, unsigned lit)
{
  ...
  assert(!f->eliminated);
  solver->active++;
  kissat_enqueue(solver, idx); // add the variable of lit to the solver->queue
  // MAB
  if (solver->mab)
  {
  ...
  solver->unassigned++;
  kissat_mark_removed_literal(solver, lit); // activate the eliminate flag for the variable idx
  kissat_mark_added_literal(solver, lit);   // activate the subsume flag for the variable idx
