...
#define ENLARGE_STACK(S)                                                                                               \
	do {                                                                                                               \
		assert(FULL_STACK(S));                                                                                         \
		kissat_mab_stack_enlarge(solver, (chars*)&(S), sizeof *(S).begin);                                             \
	} while (0)

#define SHRINK_STACK(S)                                                                                                \
	do {                                                                                                               \
		if (!FULL_STACK(S))                                                                                            \
			kissat_mab_shrink_stack(solver, (chars*)&(S), sizeof *(S).begin);                                          \
	} while (0)
...
#define RELEASE_STACK(S)                                                                                               \
	do {                                                                                                               \
		kissat_mab_dealloc(solver, (S).begin, CAPACITY_STACK(S), sizeof *(S).begin);                                   \
		INIT_STACK(S);                                                                                                 \
	} while (0)
...
void
kissat_mab_stack_enlarge(struct kissat*, chars*, size_t size_of_element);
void
kissat_mab_shrink_stack(struct kissat*, chars*, size_t size_of_element);
