...
void
kissat_mab_stack_enlarge (struct kissat *solver, chars * s, size_t bytes)
{
  ...
  if (old_bytes)
    ...
    {
      new_bytes = bytes;
      while (!kissat_mab_aligned_word (new_bytes))
	new_bytes <<= 1;
    }
  s->begin = kissat_mab_realloc (solver, s->begin, old_bytes, new_bytes);
  s->allocated = s->begin + new_bytes;
  s->end = s->begin + size;
}

void
kissat_mab_shrink_stack (struct kissat *solver, chars * s, size_t bytes)
{
  assert (bytes > 0);
  const size_t old_bytes_capacity = CAPACITY_STACK (*s);
  assert (kissat_mab_aligned_word (old_bytes_capacity));
  assert (!(old_bytes_capacity % bytes));
  assert (kissat_mab_is_zero_or_power_of_two (old_bytes_capacity / bytes));
  ...
  if (old_size)
    {
      const unsigned ld_old_size = kissat_mab_ldceil (old_size);
      new_capacity = ((size_t) 1) << ld_old_size;
    }
  else
    new_capacity = 0;
  assert (kissat_mab_is_zero_or_power_of_two (new_capacity));
  size_t new_bytes_capacity = new_capacity * bytes;
  while (!kissat_mab_aligned_word (new_bytes_capacity))
    new_bytes_capacity <<= 1;
  if (new_bytes_capacity == old_bytes_capacity)
    return;
  assert (new_bytes_capacity < old_bytes_capacity);
  s->begin = kissat_mab_realloc (solver, s->begin,
