...
kissat_mab_litpair(unsigned lit, unsigned other)
{
	litpair res;
...
static inline watch
kissat_mab_binary_watch(unsigned lit, bool redundant, bool hyper)
{
...
static inline watch
kissat_mab_large_watch(reference ref)
{
	watch res;
	res.large.ref = ref;
...
static inline watch
kissat_mab_blocking_watch(unsigned lit)
{
	watch res;
...
#define PUSH_WATCHES(W, E)                                                                                             \
	do {                                                                                                               \
		...
		kissat_mab_push_vectors(solver, &solver->vectors, &(W), (E).raw);                                              \
	} while (0)

#define LAST_WATCH_POINTER(WS) (watch*)kissat_mab_last_vector_pointer(&solver->vectors, &WS)

#define BEGIN_WATCHES(WS) ((union watch*)kissat_mab_begin_vector(&solver->vectors, &(WS)))

#define END_WATCHES(WS) ((union watch*)kissat_mab_end_vector(&solver->vectors, &(WS)))

#define RELEASE_WATCHES(WS) kissat_mab_release_vector(solver, &solver->vectors, &(WS))

#define SET_END_OF_WATCHES(WS, P)                                                                                      \
	do {                                                                                                               \
		sector SIZE = (unsigned*)(P)-kissat_mab_begin_vector(&solver->vectors, &WS);                                   \
		kissat_mab_resize_vector(solver, &solver->vectors, &WS, SIZE);                                                 \
	} while (0)

#define REMOVE_WATCHES(W, E) kissat_mab_remove_from_vector(solver, &solver->vectors, &(W), (E).raw)
...
void
kissat_mab_remove_blocking_watch(struct kissat*, watches*, reference);

void
kissat_mab_flush_large_watches(struct kissat*);
void
kissat_mab_watch_large_clauses(struct kissat*);

void
kissat_mab_connect_irredundant_large_clauses(struct kissat*);

void
kissat_mab_flush_large_connected(struct kissat*);
