...
{
#ifndef QUIET
  assert (kissat_mab_verbosity (0) < 0);
#endif
  kissat_mab_warning (0, "YOU SHOULD NOT SEE THIS WARNING!");
  kissat_mab_signal (0, "YOU SHOULD NOT SEE THIS SIGNAL MESSAGE FOR ", 0);
}
...
#endif
  kissat *solver = new_solver ();
  kissat_mab_enlarge_variables (solver, 17);
  for (int i = 1; i < (tissat_big ? 28 : 21); i++)
    {
      kissat_mab_enlarge_variables (solver, solver->vars + 1);
      kissat_mab_enlarge_variables (solver, 1 << i);
#ifndef QUIET
      printf ("after %d << 20 %s\n", i,
	      kissat_mab_format_bytes (&format,
				   kissat_mab_maximum_resident_set_size ()));
#endif
    }
  if (tissat_big)
    {
      kissat_mab_enlarge_variables (solver, EXTERNAL_MAX_VAR);
#ifndef QUIET
      printf ("after %d %s\n", EXTERNAL_MAX_VAR,
	      kissat_mab_format_bytes (&format,
				   kissat_mab_maximum_resident_set_size ()));
...
{
  kissat *solver = new_solver ();
  (void) kissat_mab_import_literal (solver, -1);
  (void) kissat_mab_import_literal (solver, 42);
  if (tissat_big)
    (void) kissat_mab_import_literal (solver, EXTERNAL_MAX_VAR);
  kissat_mab_release (solver);
}
...
test_init_add_release (void)
{
  ...
  for (int check = 0; check <= max_check; check++)
    {
      kissat *solver = new_solver ();
      kissat_mab_set_option (solver, "log", 4);
#ifndef NDEBUG
      kissat_mab_set_option (solver, "check", check);
#endif
      kissat_mab_add (solver, 2);
      kissat_mab_add (solver, -12);
      kissat_mab_add (solver, 0);
      kissat_mab_add (solver, -17);
      kissat_mab_add (solver, -2);
      kissat_mab_add (solver, 0);
      ...
      kissat_mab_add (solver, 1);
      kissat_mab_add (solver, 0);
      kissat_mab_add (solver, 7);
      kissat_mab_add (solver, 8);
      kissat_mab_add (solver, 7);
      kissat_mab_add (solver, 8);
      kissat_mab_add (solver, 7);
      kissat_mab_add (solver, 0);
      ...
      kissat_mab_add (solver, -7);
      kissat_mab_add (solver, 0);
      kissat_mab_add (solver, 9);
      kissat_mab_add (solver, 7);
      kissat_mab_add (solver, 0);
      if (tissat_big)
	{
	  kissat_mab_add (solver, EXTERNAL_MAX_VAR);
	  kissat_mab_add (solver, 0);
	}
      kissat_mab_release (solver);
    }
