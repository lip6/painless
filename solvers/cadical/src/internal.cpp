...
Internal::~Internal () {
  // printf ("In %lu phase decisions: %lu forced, %lu initial, %lu saved, "
  //         "%lu inversed, %lu target\n",
  //         totalPhaseDecisions, forcedPhaseDecisions, initialPhaseDecisions,
  //         savedPhaseDecisions, inversedPhaseDecisions,
  //         targetPhaseDecisions);
...
bool Internal::importing () {
  return level == 0 && external->learner != 0 && watching () &&
         external->learner->hasClauseToImport ();
}

void Internal::import_redundant_clauses (int &res) {
  // Another safeguard
  if (external->learner == 0)
    return;
  // A solution was found
  if (res != 0)
    return;

  std::vector<int> externClause (0);
  std::vector<int> &internClause = this->clause;

  assert (internClause.empty ());
  int glue;
  int unitClause;
  while (external->learner->hasClauseToImport ()) {
    externClause.clear ();
    external->learner->getClauseToImport (externClause, glue);
    unitClause = 0;
    assert (externClause.size () > 0);
    assert (glue >= 0);

    // Check if the clause can be imported
    bool addClause = true;
    for (auto externLit : externClause) {
      assert (externLit != 0);
      if (external->marked (external->witness, externLit)) {
        // Literal marked as witness: Cannot import
        addClause = false;
        break;
      }

      int internLit = external->internalize (externLit);

      CaDiCaL::Flags &litFlags = flags (internLit);

      if (litFlags.eliminated () || litFlags.substituted ()) {
        // Literal eliminated, no import
        addClause = false;
        break;
      } else if (litFlags.fixed ()) {
        if (val (internLit) == 1) {
          // clause is root satisfied no need to import
          addClause = false;
          break;
        } // else is false thus no need to push it
      } else // unassigned literal
      {
        internClause.push_back (internLit);
        unitClause = internLit;
      }
    }
    if (!addClause) {
      // Clause shouldn't be imported
      internClause.clear ();
      continue;
    }
    // Non Unit Clause
    if (internClause.size () >= 2) {
      // as in internal::new_learned_redundant_clause()
      external->check_learned_clause ();
      CaDiCaL::Clause *ref = new_clause (
          glue > 0,
          glue); // if glue == 0, clause is irredundant (permanent)
      if (proof)
        proof->add_derived_clause (ref,
                                   {}); // lrat_chain can be also share for
                                        // parallel proof generation ?
      assert (watching ());
      watch_clause (ref);
      internClause.clear ();
    } else if (internClause.size () == 1) /*Unit clause*/ {
      /* Checks if UNSAT */
      assert (original.empty ());
      assign_unit (unitClause);
      internClause.clear ();
    }
  }
}
