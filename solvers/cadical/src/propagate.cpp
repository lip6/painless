  ...
  while (!conflict && propagated != trail.size ()) {
    ...
    while (i != eow) {
      ...
      if (w.binary ()) {
        ...
        if (w.clause->garbage) {
          // In order to overwrite the current watch with the next one
          j--;
          continue;
        }
        ...
        if (u > 0)
          ...
          // current loops will stop at an unassigned even though a satisfying lit may exist
    ...
    // Some watches were dropped
    if (j != i) {
      // since we break when we see a conflict
      while (i != eow)
        *j++ = *i++;
