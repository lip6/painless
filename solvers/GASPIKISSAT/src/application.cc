...
parsed_one_option_and_return_zero_exit_code(char *arg)
{
  ...
  {
    gaspi::kissat_banner(0, "KISSAT SAT Solver");
    return true;
  }
  if (!strcmp(arg, "--compiler"))
  {
    printf("%s\n", gaspi::kissat_compiler());
    return true;
  }
#ifndef NOPTIONS
...
parse_options(application *application, int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (!strcmp(arg, "-l"))
    {
      ...
      if (value < INT_MAX)
        value++;
      gaspi::kissat_set_option(solver, "log", value);
    }
#endif
      ...
      gaspi::kissat_set_option(solver, "quiet", 1);
    else if (!strcmp(arg, "-s"))
      gaspi::kissat_set_option(solver, "statistics", 1);
    else if (!strcmp(arg, "-v"))
    {
      int value = GET_OPTION(verbose);
      if (value < INT_MAX)
        value++;
      gaspi::kissat_set_option(solver, "verbose", value);
    }
#endif
    ...
    else if ((valstr = kissat_parse_option_name(arg, "conflicts")))
    {
      ...
      if (kissat_parse_option_value(valstr, &val) && val >= 0)
      {
        ...
        gaspi::kissat_set_conflict_limit(solver, val);
    ...
    else if ((valstr = kissat_parse_option_name(arg, "decisions")))
    {
      ...
      if (kissat_parse_option_value(valstr, &val) && val >= 0)
      {
        ...
        gaspi::kissat_set_decision_limit(solver, val);
             ...
             kissat_has_configuration(arg + 2))
    {
      if (configuration)
        ERROR("multiple configurations '%s' and '%s'",
              configuration, arg);
      gaspi::kissat_set_configuration(solver, arg + 2);
      configuration = arg;
    }
    else if (arg[0] == '-' && arg[1] == '-')
    {
      ...
      gaspi::kissat_set_option(solver, name, value);
    }
#else
#ifdef SAT
...
#if !defined(QUIET) && !defined(NOPTIONS)
  if (gaspi::kissat_get_option(solver, "quiet"))
  {
    if (gaspi::kissat_get_option(solver, "statistics"))
      ERROR("can not use '--quiet' ('-q') with '--statistics' ('-s')");
    if (gaspi::kissat_get_option(solver, "verbose"))
...
parse_input(application *application)
{
  ...
  kissat_message(solver, "");
  const char *error = gaspi::kissat_parse_dimacs(solver, application->strict, &file,
  ...
  for (const int *p = begin; p != end; p = q + 1)
    {
      ...
      int lit;
      for (q = p; (lit = *q); q++)
  if (!satisfied && gaspi::kissat_value(solver, lit) == lit)
    satisfied = true;
...
int kissat_application(kissat *solver, int argc, char **argv)
{
  ...
  if (!GET_OPTION(quiet))
    gaspi::kissat_banner("c ", "KISSAT SAT Solver");
#endif
#ifndef NPROOFS
...
#ifndef QUIET
  gaspi::kissat_print_statistics(solver);
#endif
#ifndef NPROOFS
  close_proof(&application);
#endif
