...
#define OPTIONS                                                                \
  ...
  OPTION(crossover, 50, 0, 100, "crossover rate in percent")                   \
...
extern const opt *gkissat_options_begin;
extern const opt *gkissat_options_end;

#define all_options(O)                 \
  const opt *O = gkissat_options_begin; \
  O != gkissat_options_end;             \
  ++O
...
kissat_options_ref(const options *options, const opt *o)
{
  if (!o)
    return 0;
  assert(gkissat_options_begin <= o);
  assert(o < gkissat_options_end);
  return (int *)options + (o - gkissat_options_begin);
}
