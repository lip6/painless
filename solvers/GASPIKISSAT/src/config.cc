...
void
gaspi::kissat_set_configuration(kissat * solver, const char *name)
{
  if (!strcmp (name, "default"))
    return;
  if (!strcmp (name, "sat"))
    {
      gaspi::kissat_set_option(solver, "target", TARGET_SAT);
      return;
    }
  if (!strcmp (name, "unsat"))
    {
      gaspi::kissat_set_option(solver, "stable", STABLE_UNSAT);
      return;
    }
}
