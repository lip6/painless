...
learn_unit (kissat * solver)
{
  ...
  kissat_inc_backtrack (solver, new_level);
  kissat_inc_assign_unit (solver, not_uip);
...
learn_binary (kissat * solver)
{
  ...
  const reference ref =
#endif
    kissat_inc_new_redundant_clause (solver, glue);
  assert (ref == INVALID_REF);
  kissat_inc_assign_binary (solver, true, not_uip, other);
  kissat_inc_eager_subsume (solver);
}
...
learn_reference (kissat * solver)
{
  ...
  *q = lits[1];
  lits[1] = jump_lit;
  const unsigned glue = SIZE_STACK (solver->levels);
  const reference ref = kissat_inc_new_redundant_clause (solver, glue);
  assert (ref != INVALID_REF);
  clause *c = kissat_inc_dereference_clause (solver, ref);
  c->used = 1 + (glue <= (unsigned) GET_OPTION (tier2));
  const unsigned new_level = determine_new_level (solver, jump_level);
  kissat_inc_backtrack (solver, new_level);
  kissat_inc_assign_reference (solver, not_uip, ref, c);
  kissat_inc_eager_subsume (solver);
  kissat_inc_push_clueue (&solver->clueue, ref);
}

// Begin Painless
static inline void kissat_inc_export_to_painless(kissat *solver, unsigned glue, unsigned size)
{
  ...
  else
  {
    ...
    solver->cbkExportClause(solver->issuer, solver);
  }
}
// End Painless

void
kissat_inc_learn_clause (kissat * solver)
{
  if (!solver->probing)
    INC (learned);
  if (solver->stable)
    kissat_inc_tick_reluctant (&solver->reluctant);
  const unsigned glue = SIZE_STACK (solver->levels);
  ...
  // Begin Painless
  kissat_inc_export_to_painless(solver, glue, size);
  // End Painless
}
 ...
 */
bool kissat_inc_import_unit_from_painless(kissat *solver)
{
  ...
  if (false == solver->cbkImportUnit(solver->issuer, solver))
  {
  ...
  for (all_stack(int, external_lit, solver->pclause.lits))
  {
    ...
    case 0:
      ...
      kissat_inc_assign_unit(solver, internal_lit);
      break;
    case -1:
    ...
    case 1:
      ...
      break;
    default:
      kissat_inc_fatal("ERROR, unexpected value %d for literal %d in solver %d!!", VALUE(internal_lit), external_lit, solver->id_painless);
      return false;
    }
  }
  return true;
 ...
 */
bool kissat_inc_import_from_painless(kissat *solver)
{
  ...
  while (solver->cbkImportClause(solver->issuer, solver))
  {
    int internal_lit;
    ...
    case 2:
      ...
      new_clause_ref = kissat_inc_new_redundant_clause(solver, size);
    ...
    default:
      ...
      clause *new_clause = kissat_inc_dereference_clause(solver, new_clause_ref);
      new_clause->used = 1 + (size <= (unsigned)GET_OPTION(tier2)); // not understood, yet
      kissat_inc_eager_subsume(solver);
      kissat_inc_push_clueue(&solver->clueue, new_clause_ref); // not understood, yet
    }
  }
