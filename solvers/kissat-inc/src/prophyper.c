...
{
  const watch watch = kissat_inc_binary_watch (other, true, true);
...
binary_propagate_literal (kissat * solver, unsigned lit)
{
  ...
  const watch *p = begin;
  int ticks =  kissat_inc_cache_lines (watches->size, sizeof (watch));
  ...
  while (p != end)
	...
	continue;
      ...
      if (other_value < 0)
	{
	  res = kissat_inc_binary_conflict (solver, redundant, not_lit, other);
	  break;
	}
      assert (!other_value);
      kissat_inc_assign_binary (solver, values, assigned, redundant, other,
			    not_lit);
    }
  return res;
}
...
clause *
kissat_inc_hyper_propagate (kissat * solver, const clause * ignore)
{
  assert (solver->probing);
  assert (solver->watching);
  ...
  kissat_inc_update_probing_propagation_statistics (solver, propagated);
