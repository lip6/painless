...
void
kissat_inc_reset_rephased (kissat * solver)
{
...
void
kissat_inc_reset_target_assigned (kissat * solver)
{
...
void
kissat_inc_reset_consistently_assigned (kissat * solver)
{
...
bool
kissat_inc_rephasing (kissat * solver)
{
...
char
kissat_inc_rephase_best (kissat * solver)
{
...
{
  for (all_phases (p))
    p->saved = (kissat_inc_pick_bool (&solver->random) ? 1 : -1);
  return '#';
}
...
{
  STOP (rephase);
  char res = kissat_inc_walk (solver);
  if (res == 'W')
    kissat_inc_autarky (solver);
  START (rephase);
  if (!res)
...
{
  // kissat_inc_clear_target_phases (solver);
  kissat_inc_reset_target_assigned (solver);
  ...
  kissat_inc_phase (solver, "rephase", GET (rephased),
...
void
kissat_inc_rephase (kissat * solver)
{
