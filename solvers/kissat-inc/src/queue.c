...
void
kissat_inc_init_queue (queue * queue)
{
...
{
  kissat_inc_very_verbose (solver, "need to reassign enqueue stamps on queue");
#ifdef QUIET
  (void) solver;
#endif
  queue->stamp = 0;
...
void
kissat_inc_check_queue (kissat * solver)
{
...
#else
#define kissat_inc_check_queue(...) do { } while (0)
#endif

void
kissat_inc_enqueue (kissat * solver, unsigned idx)
{
  ...
  if (!VALUE (LIT (idx)))
    kissat_inc_update_queue (solver, links, idx);
  kissat_inc_check_queue (solver);
}

void
kissat_inc_dequeue (kissat * solver, unsigned idx)
{
  assert (idx < solver->vars);
  ...
  if (solver->queue.search.idx == idx)
	...
	kissat_inc_update_queue (solver, links, search);
    }
...
void
kissat_inc_move_to_front (kissat * solver, unsigned idx)
{
  queue *queue = &solver->queue;
  links *links = solver->links;
  if (idx == queue->last)
  ...
  if (tmp && queue->search.idx == idx)
    {
      ...
      if (!DISCONNECTED (prev))
	kissat_inc_update_queue (solver, links, prev);
      else
	{
	  unsigned next = links[idx].next;
	  assert (!DISCONNECTED (next));
	  kissat_inc_update_queue (solver, links, next);
	}
    }
