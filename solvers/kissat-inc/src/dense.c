		  ...
		  if (lit < other)
		    {
		      ...
		      kissat_inc_delete_binary (solver, red, hyper, lit, other);
		      collected++;
		    }
		}
	    }
...
void
kissat_inc_enter_dense_mode (kissat * solver,
			 litpairs * irredundant, litwatches * redundant)
{
  assert (!solver->level);
  assert (solver->watching);
					    ...
					    litwatches * binaries)
{
  ...
  for (all_stack (litwatch, litwatch, *binaries))
      ...
      else
	{
	  ...
	  const bool hyper = watch.binary.hyper;
	  kissat_inc_delete_binary (solver, redundant, hyper, first, second);
#ifdef LOGGING
	  flushed_eliminated++;
#endif
	}
  ...
  for (all_stack (litpair, litpair, *binaries))
    {
      ...
      watches *first_watches = all_watches + first;
      watch first_watch = kissat_inc_binary_watch (second, false, false);
      PUSH_WATCHES (*first_watches, first_watch);

      watches *second_watches = all_watches + second;
      watch second_watch = kissat_inc_binary_watch (first, false, false);
...
void
kissat_inc_resume_sparse_mode (kissat * solver, bool flush_eliminated,
			   litpairs * irredundant, litwatches * redundant)
{
  assert (!solver->level);
  assert (!solver->watching);
  ...
  if (solver->probing)
    ...
    conflict = kissat_inc_search_propagate (solver);
  if (conflict)
    {
