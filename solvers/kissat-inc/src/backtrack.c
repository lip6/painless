				      ...
				      heap * scores, unsigned lit)
{
  assert (solver->stable);
  const unsigned idx = IDX (lit);
  if (!kissat_inc_heap_contains (scores, idx))
...
update_phases (kissat * solver)
{
  ...
  if (!solver->probing)
      ...
      if (solver->target_assigned < solver->consistently_assigned)
	{
	  ...
	  solver->target_assigned = solver->consistently_assigned;
	  kissat_inc_save_target_phases (solver);
	}
      ...
      if (solver->best_assigned < solver->consistently_assigned)
	{
	  ...
	  solver->best_assigned = solver->consistently_assigned;
	  kissat_inc_save_best_phases (solver);
	}
...
void
kissat_inc_backtrack (kissat * solver, unsigned new_level)
{
...
void
kissat_inc_backtrack_propagate_and_flush_trail (kissat * solver)
{
  if (solver->level)
    {
