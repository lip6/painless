...
void
kissat_inc_clear_clueue (kissat * solver, clueue * clueue)
{
  if (!clueue->size)
    return;
...
void
kissat_inc_release_clueue (kissat * solver, clueue * clueue)
{
  if (!clueue->size)
    return;
  const size_t bytes = clueue->size * sizeof *clueue->elements;
  kissat_inc_free (solver, clueue->elements, bytes);
}

void
kissat_inc_init_clueue (kissat * solver, clueue * clueue, unsigned size)
{
  assert (size);
  assert (!clueue->size);
  clueue->size = size;
  clueue->elements = kissat_inc_malloc (solver, size * sizeof *clueue->elements);
  assert (!clueue->next);
  LOG ("initialized clueue of size %u", size);
  kissat_inc_clear_clueue (solver, clueue);
}
...
void
kissat_inc_eager_subsume (kissat * solver)
{
