...
last_enqueued_unassigned_variable (kissat * solver)
{
  ...
  if (values[LIT (res)])
      ...
      do
	{
      ...
      kissat_inc_update_queue (solver, links, res);
    }
#ifdef LOGGING
...
{
  unsigned res = kissat_inc_max_heap (heap);
  const value *values = solver->values;
  while (values[LIT (res)])
    {
      kissat_inc_pop_heap (solver, heap, res);
      res = kissat_inc_max_heap (heap);
    }
  ...
  const double score = kissat_inc_get_heap_score (heap, res);
#endif
  ...
  for (all_variables (idx))
    {
      if (VALUE (LIT (idx)))
	continue;
      const double idx_score = kissat_inc_get_heap_score (heap, idx);
...
unsigned
kissat_inc_next_decision_variable (kissat * solver)
{
  unsigned res;
  if (solver->stable)
...
void
kissat_inc_decide (kissat * solver)
{
  START (decide);
  assert (solver->unassigned);
  INC (decisions);
  ...
  solver->level++;
  const unsigned idx = kissat_inc_next_decision_variable (solver);
  const value value = decide_phase (solver, idx);
  unsigned lit = LIT (idx);
  if (value < 0)
    lit = NOT (lit);
  kissat_inc_push_frame (solver, lit);
  assert (solver->level < SIZE_STACK (solver->frames));
  LOG ("decide literal %s", LOGLIT (lit));
  kissat_inc_assign_decision (solver, lit);
  STOP (decide);
}

void
kissat_inc_internal_assume (kissat * solver, unsigned lit)
{
  assert (solver->unassigned);
  assert (!VALUE (lit));
  assert (solver->level < MAX_LEVEL);
