...
bool
kissat_inc_find_and_gate (kissat * solver, unsigned lit, unsigned negative)
{
  if (!GET_OPTION (ands))
    return false;
  size_t marked = kissat_inc_mark_binaries (solver, lit);
  if (!marked)
    return false;
  if (marked < 2)
    {
      kissat_inc_unmark_binaries (solver, lit);
      return false;
    }
  ...
  for (all_binary_large_watches (watch, *not_watches))
      ...
      for (all_literals_in_clause (other, c))
	{
	  ...
	  if (value > 0)
	    {
	      kissat_inc_eliminate_clause (solver, c, INVALID_LIT);
	      base = 0;
	      break;
	    }
	  if (value < 0)
  ...
  watch tmp = kissat_inc_binary_watch (0, false, false);
  ...
  tmp = kissat_inc_large_watch (kissat_inc_reference_clause (solver, base));
  PUSH_STACK (solver->gates[!negative], tmp);
