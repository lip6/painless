...
void
kissat_inc_statistics_print (kissat * solver, bool verbose)
{
#ifndef QUIET
  statistics *statistics = &solver->statistics;

  const double time = kissat_inc_process_time ();
  size_t variables = SIZE_STACK (solver->import);
#ifndef NMETRICS
  const double rss = kissat_inc_maximum_resident_set_size ();
#endif

/*------------------------------------------------------------------------*/

#define RELATIVE(FIRST,SECOND) \
  kissat_inc_average (statistics->FIRST, statistics->SECOND)
...
#define PER_SECOND(NAME) \
  kissat_inc_average (statistics->NAME, time)
...
#define PER_VARIABLE(NAME) \
  kissat_inc_average (statistics->NAME, variables)
...
#define PERCENT(FIRST,SECOND) \
  kissat_inc_percent (statistics->FIRST, statistics->SECOND)
...
#define PCNT_RESIDENT_SET(NAME) \
  kissat_inc_percent (statistics->NAME, rss)
...
#define PCNT_VARIABLES(NAME) \
  kissat_inc_percent (statistics->NAME, variables)
...
#elif defined(NDEBUG)
int kissat_inc_statistics_dummy_to_avoid_warning;
#endif
...
void
kissat_inc_check_statistics (kissat * solver)
{
  ...
  for (all_clauses (c))
    {
      if (c->garbage)
	{
	  arena_garbage += kissat_inc_actual_bytes_of_clause (c);
	  continue;
	}
      if (c->hyper)
	hyper_ternaries++;
