...
void kissat_inc_dense_collect (struct kissat *);
void kissat_inc_sparse_collect (struct kissat *, bool compact, reference start);

static inline void
kissat_inc_defrag_watches_if_needed (kissat * solver)
{
  ...
  kissat_inc_defrag_vectors (solver, &solver->vectors, LITS, solver->watches);
}
