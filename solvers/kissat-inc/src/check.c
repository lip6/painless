...
void
kissat_inc_check_satisfying_assignment (kissat * solver)
{
...
init_nonces (kissat * solver, checker * checker)
{
  ...
  for (unsigned i = 0; i < MAX_NONCES; i++)
    checker->nonces[i] = 1 | kissat_inc_next_random32 (&random);
...
void
kissat_inc_init_checker (kissat * solver)
{
  LOG ("initializing internal proof checker");
  checker *checker = kissat_inc_calloc (solver, 1, sizeof (struct checker));
...
release_hash (kissat * solver, checker * checker)
{
  for (unsigned h = 0; h < checker->hashed; h++)
	...
	{
	  next = line->next;
	  kissat_inc_free (solver, line, bytes_line (line->size));
	}
    }
  kissat_inc_dealloc (solver, checker->table, checker->hashed, sizeof (line *));
}
...
release_watches (kissat * solver, checker * checker)
{
  ...
  kissat_inc_dealloc (solver, checker->watches, 2 * checker->size,
		  sizeof (lines));
}

void
kissat_inc_release_checker (kissat * solver)
{
  ...
  kissat_inc_free (solver, checker->marks, 2 * checker->size * sizeof (bool));
  kissat_inc_free (solver, checker->values, 2 * checker->size);
  release_watches (solver, checker);
  kissat_inc_free (solver, checker, sizeof (struct checker));
}
...
#define PERCENT_ADDED(NAME) \
  kissat_inc_percent (checker->NAME, checker->added)

void
kissat_inc_print_checker_statistics (kissat * solver, bool verbose)
{
  checker *checker = solver->checker;
  if (verbose)
  ...
  if (verbose)
    PRINT_STAT ("checker_collisions", checker->collisions,
		kissat_inc_percent (checker->collisions, checker->searches),
		"%", "per search");
  PRINT_STAT ("checker_decisions", checker->decisions,
	      kissat_inc_average (checker->decisions, checker->checked),
	      "", "per check");
  PRINT_STAT ("checker_propagations", checker->propagations,
	      kissat_inc_average (checker->propagations, checker->checked),
	      "", "per check");
...
resize_hash (kissat * solver, checker * checker)
{
  ...
  line **table = kissat_inc_calloc (solver, new_hashed, sizeof (line *));
  ...
  kissat_inc_dealloc (solver, checker->table, old_hashed, sizeof (line *));
...
{
  line *res = kissat_inc_malloc (solver, bytes_line (size));
  res->next = 0;
  res->size = size;
  res->hash = hash;
...
resize_checker (kissat * solver, checker * checker, unsigned new_vars)
{
  ...
  if (new_vars > size)
	...
	new_size *= 2;
      ...
      checker->marks =
	kissat_inc_realloc (solver, checker->marks, size2,
			new_size2 * sizeof (bool));
      checker->values =
	kissat_inc_realloc (solver, checker->values, size2, new_size2);
      checker->watches =
	kissat_inc_realloc (solver, checker->watches,
...
{
  const int elit = kissat_inc_export_literal (solver, ilit);
...
remove_line (kissat * solver, checker * checker, size_t size)
{
  ...
  kissat_inc_free (solver, line, bytes_line (size));
...
{
  if (size > UINT_MAX)
    kissat_inc_fatal ("can not check handle original clause of size %zu", size);
...
line_redundant (kissat * solver, checker * checker, size_t size)
{
  ...
  if (!size)
    kissat_inc_fatal ("checker can not remove empty checker clause");
  if (size == 1)
    {
	...
	kissat_inc_fatal ("consistent checker can not remove falsified unit %d",
		      export_checker (checker, unit));
      if (!value)
	kissat_inc_fatal ("checker can not remove unassigned unit %d",
		      export_checker (checker, unit));
...
void
kissat_inc_add_unchecked_external (kissat * solver, size_t size, int *elits)
{
...
void
kissat_inc_add_unchecked_internal (kissat * solver, size_t size, unsigned *lits)
{
...
void
kissat_inc_check_and_add_binary (kissat * solver, unsigned a, unsigned b)
{
...
void
kissat_inc_check_and_add_clause (kissat * solver, clause * clause)
{
...
void
kissat_inc_check_and_add_empty (kissat * solver)
{
...
void
kissat_inc_check_and_add_internal (kissat * solver, size_t size, unsigned *lits)
{
...
void
kissat_inc_check_and_add_unit (kissat * solver, unsigned a)
{
...
void
kissat_inc_check_shrink_clause (kissat * solver, clause * c,
...
void
kissat_inc_remove_checker_binary (kissat * solver, unsigned a, unsigned b)
{
...
void
kissat_inc_remove_checker_clause (kissat * solver, clause * clause)
{
...
bool
kissat_inc_checker_contains_clause (kissat * solver, clause * clause)
{
  checker *checker = solver->checker;
  import_clause (solver, checker, clause);
  size_t size = SIZE_STACK (checker->imported);
...
void
kissat_inc_remove_checker_external (kissat * solver, size_t size, int *elits)
{
...
void
kissat_inc_remove_checker_internal (kissat * solver, size_t size, unsigned *ilits)
{
...
#else
int kissat_inc_check_dummy_to_avoid_warning;
#endif
