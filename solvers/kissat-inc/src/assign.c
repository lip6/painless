...
static inline void
kissat_inc_assign (kissat * solver,
#ifdef INLINE_ASSIGN
	       ...
	       bool binary, bool redundant, unsigned level, unsigned reason)
{
  ...
  if (!level)
    {
      kissat_inc_mark_fixed_literal (solver, lit);
...
kissat_inc_assignment_level (kissat * solver,
			 value * values, assigned * assigned,
...
void
kissat_inc_assign_unit (kissat * solver, unsigned lit)
{
  kissat_inc_assign (solver, lit, false, false, 0, UNIT);
  LOGUNARY (lit, "assign %s reason", LOGLIT (lit));
}

void
kissat_inc_assign_decision (kissat * solver, unsigned lit)
{
  kissat_inc_assign (solver, lit, false, false, solver->level, DECISION);
...
#endif
  void
kissat_inc_assign_binary (kissat * solver,
#ifdef INLINE_ASSIGN
		      value * values, assigned * assigned,
#endif
...
#endif
  void
kissat_inc_assign_reference (kissat * solver,
#ifdef INLINE_ASSIGN
			 value * values, assigned * assigned,
#endif
			 unsigned lit, reference ref, clause * reason)
{
  assert (reason == kissat_inc_dereference_clause (solver, ref));
#ifndef INLINE_ASSIGN
  assigned *assigned = solver->assigned;
  value *values = solver->values;
#endif
  const unsigned level =
    kissat_inc_assignment_level (solver, values, assigned, lit, reason);
  assert (ref != DECISION);
  assert (ref != UNIT);
  kissat_inc_assign (solver,
#ifdef INLINE_ASSIGN
