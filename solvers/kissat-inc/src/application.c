...
print_complete_usage (void)
{
  ...
  printf ("\n");
  kissat_inc_configuration_usage ();
  printf ("\n");
  printf ("Or '<option>' is one of the following long options:\n\n");
  kissat_inc_options_usage ();
#else
...
parsed_one_option_and_return_zero_exit_code (char *arg)
{
  ...
  if (!strcmp (arg, "--banner"))
    {
      kissat_inc_banner (0, "KISSAT SAT Solver");
      return true;
    }
  ...
  if (!strcmp (arg, "--embedded"))
    {
      kissat_inc_print_embedded_option_list ();
      return true;
    }
#endif
  if (!strcmp (arg, "--id"))
  ...
  if (!strcmp (arg, "--range"))
    {
      kissat_inc_print_option_range_list ();
      return true;
    }
#endif
...
{
  if (!kissat_inc_file_readable (path))
    return false;

  if (kissat_inc_has_suffix (path, ".dimacs"))
    return true;
  if (kissat_inc_has_suffix (path, ".dimacs.7z"))
    return true;
  if (kissat_inc_has_suffix (path, ".dimacs.bz2"))
    return true;
  if (kissat_inc_has_suffix (path, ".dimacs.gz"))
    return true;
  if (kissat_inc_has_suffix (path, ".dimacs.lzma"))
    return true;
  if (kissat_inc_has_suffix (path, ".dimacs.xz"))
    return true;

  if (kissat_inc_has_suffix (path, ".cnf"))
    return true;
  if (kissat_inc_has_suffix (path, ".cnf.7z"))
    return true;
  if (kissat_inc_has_suffix (path, ".cnf.bz2"))
    return true;
  if (kissat_inc_has_suffix (path, ".cnf.gz"))
    return true;
  if (kissat_inc_has_suffix (path, ".cnf.lzma"))
    return true;
  if (kissat_inc_has_suffix (path, ".cnf.xz"))
    return true;
...
parse_options (application * application, int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
      ...
      else if (!strcmp (arg, "-l"))
	{
	  ...
	  if (value < INT_MAX)
	    value++;
	  kissat_inc_set_option (solver, "log", value);
	}
#endif
	...
	kissat_inc_set_option (solver, "quiet", 1);
      else if (!strcmp (arg, "-s"))
	kissat_inc_set_option (solver, "statistics", 1);
      else if (!strcmp (arg, "-v"))
	{
	  int value = GET_OPTION (verbose);
	  if (value < INT_MAX)
	    value++;
	  kissat_inc_set_option (solver, "verbose", value);
	}
#endif
      else if (!strcmp (arg, "--color") ||
	       !strcmp (arg, "--colors") ||
	       !strcmp (arg, "--colour") || !strcmp (arg, "--colours"))
	kissat_inc_force_colors ();
      else if (!strcmp (arg, "--no-color") ||
	       !strcmp (arg, "--no-colors") ||
	       !strcmp (arg, "--no-colour") || !strcmp (arg, "--no-colours"))
	kissat_inc_force_no_colors ();
      else if ((valstr = kissat_inc_parse_option_name (arg, "time")))
	{
	  int val;
	  if (kissat_inc_parse_option_value (valstr, &val) && val > 0)
	    {
      ...
      else if ((valstr = kissat_inc_parse_option_name (arg, "conflicts")))
	{
	  int val;
	  if (kissat_inc_parse_option_value (valstr, &val) && val >= 0)
	    {
		...
		ERROR ("multiple '--conflicts=%d' and '%s'",
	      ...
	      kissat_inc_set_conflict_limit (solver, val);
	      application->conflicts = val;
	    }
	  else
	    ERROR ("invalid argument in '%s' (try '-h')", arg);
	}
      else if ((valstr = kissat_inc_parse_option_name (arg, "decisions")))
	{
	  int val;
	  if (kissat_inc_parse_option_value (valstr, &val) && val >= 0)
	    {
		...
		ERROR ("multiple '--decisions=%d' and '%s'",
	      ...
	      kissat_inc_set_decision_limit (solver, val);
	...
	application->binary = -1;
	       ...
	       kissat_inc_has_configuration (arg + 2))
	{
	  if (configuration)
	    ERROR ("multiple configurations '%s' and '%s'",
		   configuration, arg);
	  kissat_inc_set_configuration (solver, arg + 2);
	  configuration = arg;
	}
      else if (arg[0] == '-' && arg[1] == '-')
	{
	  char name[kissat_inc_options_max_name_buffer_size];
	  int value;
	  if (!kissat_inc_options_parse_arg (arg, name, &value))
	    ERROR ("invalid long option '%s' (try '-h')", arg);
	  kissat_inc_set_option (solver, name, value);
	}
#else
#ifdef SAT
      ...
      else if (application->input_path)
	{
	  ...
	  if (!kissat_inc_file_writable (arg))
	...
	{
	  if (!kissat_inc_file_readable (arg))
	    ERROR ("can not read '%s'", arg);
	  application->input_path = arg;
	}
    }
#if !defined(QUIET) && !defined(NOPTIONS)
  if (kissat_inc_get_option (solver, "quiet"))
    {
      if (kissat_inc_get_option (solver, "statistics"))
	ERROR ("can not use '--quiet' ('-q') with '--statistics' ('-s')");
      if (kissat_inc_get_option (solver, "verbose"))
...
{
#ifndef QUIET
  double entered = kissat_inc_process_time ();
#endif
  kissat *solver = application->solver;
  uint64_t lineno;
  file file;
  ...
  if (!path)
    kissat_inc_read_already_open_file (&file, stdin, "<stdin>");
  else if (!kissat_inc_open_to_read_file (&file, path))
    ERROR ("failed to open '%s' for reading", path);
  kissat_inc_section (solver, "parsing");
  kissat_inc_message (solver, "opened and reading %sDIMACS file:",
		  file.compressed ? "compressed " : "");
  kissat_inc_message (solver, "");
  kissat_inc_message (solver, "  %s", file.path);
  kissat_inc_message (solver, "");
  const char *error = kissat_inc_parse_dimacs (solver, application->strict, &file,
					   &lineno, &application->max_var);
  kissat_inc_close_file (&file);
  if (error)
    ERROR ("%s:%" PRIu64 ": parse error: %s", file.path, lineno, error);
#ifndef QUIET
  kissat_inc_message (solver, "closing input after reading %s",
		  FORMAT_BYTES (file.bytes));
  if (file.compressed)
    {
      assert (path);
      size_t bytes = kissat_inc_file_size (path);
      kissat_inc_message (solver,
		      "inflated input file of size %s by %.2f",
		      FORMAT_BYTES (bytes),
		      kissat_inc_average (file.bytes, bytes));
    }
  kissat_inc_message (solver,
		  "finished parsing after %.2f seconds",
		  kissat_inc_process_time () - entered);
#endif
  return true;
}
...
write_proof (application * application)
{
  ...
  if (!strcmp (path, "-"))
    {
      binary = false;
      kissat_inc_write_already_open_file (file, stdout, "<stdout>");
    }
  else if (!kissat_inc_open_to_write_file (file, path))
    ERROR ("failed to open and write proof to '%s'", path);
  else if (application->binary < 0)
    binary = false;
  kissat_inc_init_proof (application->solver, file, binary);
#ifndef QUIET
  kissat *solver = application->solver;
  kissat_inc_section (solver, "proving");
  kissat_inc_message (solver, "%swriting proof to %sDRAT file:",
...
close_proof (application * application)
{
  ...
  if (!path)
    return;
  kissat_inc_release_proof (application->solver);
  kissat_inc_close_file (&application->proof_file);
}
...
print_option (kissat * solver, int value, const opt * o)
{
  ...
  kissat_inc_message (solver, "--%-30s (%s default '%s')",
...
{
  const int verbosity = kissat_inc_verbosity (solver);
  if (verbosity < 0)
    return;
  size_t printed = 0;
  for (all_options (o))
    {
      const int value = *kissat_inc_options_ref (&solver->options, o);
      if (o->value != value || verbosity > 0)
	{
	  if (!printed++)
	    kissat_inc_section (solver, "options");
...
print_limits (application * application)
{
  ...
  kissat_inc_section (solver, "limits");
  if (!application->time &&
      application->conflicts < 0 && application->conflicts < 0)
    kissat_inc_message (solver, "no time, conflict nor decision limit set");
  else
    {
      if (application->time)
	kissat_inc_message (solver,
	...
	kissat_inc_message (solver, "no time limit");
	...
	kissat_inc_message (solver, "no conflict limit");
	...
	kissat_inc_message (solver, "no decision limit");
    }
}
...
/*void kissat_inc_check_a (kissat * solver)
{
  kissat_inc_message (solver, "checking assignment\n");
  ...
  for (const int *p = begin; p != end; p = q + 1)
    {
      ...
      int lit;
      for (q = p; (lit = *q); q++)
	if (!satisfied && kissat_inc_value (solver, lit) == lit)
	  satisfied = true;
    ...
    kissat_inc_message (solver, "assignment satisfies all original clauses\n");

}*/

int
kissat_inc_application (kissat * solver, int argc, char **argv)
{
  if (argc == 2)
  ...
  if (!parse_options (&application, argc, argv))
    return 1;
#ifndef QUIET
  kissat_inc_section (solver, "banner");
  if (!GET_OPTION (quiet))
    kissat_inc_banner ("c ", "KISSAT SAT Solver");
#endif
#ifndef NPROOFS
  ...
  kissat_inc_section (solver, "solving");
#endif
  int res = kissat_inc_solve (solver);
  if (res)
    {
      kissat_inc_section (solver, "result");
      if (res == 20)
	{
...
#endif
	  /*kissat_inc_check_a (solver);*/
	  printf ("s SATISFIABLE\n");
	  fflush (stdout);
	  if (application.witness)
	    kissat_inc_print_witness (solver,
...
#endif
#ifndef QUIET
  kissat_inc_section (solver, "shutting down");
  kissat_inc_message (solver, "exit %d", res);
#endif
  return res;
}
