...
\
static inline void \
kissat_inc_inc_ ## NAME (statistics * statistics) \
{ \
...
\
static inline void \
kissat_inc_dec_ ## NAME (statistics * statistics) \
{ \
...
\
static inline void \
kissat_inc_add_ ## NAME (statistics * statistics, uint64_t n) \
{ \
...
\
static inline void \
kissat_inc_sub_ ## NAME (statistics * statistics, uint64_t n) \
{ \
...
kissat_inc_get_ ## NAME (statistics * statistics) \
{ \
...
#define INC(NAME) kissat_inc_inc_ ## NAME (&solver->statistics)
#define DEC(NAME) kissat_inc_dec_ ## NAME (&solver->statistics)
#define ADD(NAME,N) kissat_inc_add_ ## NAME (&solver->statistics, (N))
#define SUB(NAME,N) kissat_inc_sub_ ## NAME (&solver->statistics, (N))
#define GET(NAME) kissat_inc_get_ ## NAME (&solver->statistics)
...
void kissat_inc_statistics_print (struct kissat *, bool verbose);
...
struct kissat;
void kissat_inc_check_statistics (struct kissat *);

#else

#define kissat_inc_check_statistics(...) do { } while (0)
