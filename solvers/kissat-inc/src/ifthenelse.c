...
{
  clause *clause = kissat_inc_dereference_clause (solver, ref);
  if (clause->garbage)
    return false;
  ...
  for (all_literals_in_clause (other, clause))
    {
      ...
      if (value > 0)
	{
	  kissat_inc_eliminate_clause (solver, clause, INVALID_LIT);
	  return false;
	}
      if (value < 0)
	continue;
...
bool
kissat_inc_find_if_then_else_gate (kissat * solver,
