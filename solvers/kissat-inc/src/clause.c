...
void
kissat_inc_connect_clause (kissat * solver, clause * c)
{
  watches *all_watches = solver->watches;
  const reference ref = kissat_inc_reference_clause (solver, c);
  kissat_inc_inlined_connect_clause (solver, all_watches, c, ref);
}
		   ...
		   unsigned first, unsigned second)
{
  ...
  kissat_inc_watch_binary (solver, redundant, false, first, second);
  if (!redundant)
    {
      kissat_inc_mark_added_literal (solver, first);
      kissat_inc_mark_added_literal (solver, second);
    }
...
{
  assert (size > 2);
  reference res = kissat_inc_allocate_clause (solver, size);
  clause *c = kissat_inc_unchecked_dereference_clause (solver, res);
  init_clause (solver, c, redundant, glue, size);
  memcpy (c->lits, lits, size * sizeof (unsigned));
  LOGREF (res, "new");
  if (solver->watching)
    kissat_inc_watch_reference (solver, lits[0], lits[1], res);
  else
  ...
  if (redundant)
    ...
    {
      kissat_inc_mark_added_literals (solver, size, lits);
	    ...
	    unsigned glue, unsigned size, unsigned *lits)
{
  ...
  kissat_inc_defrag_watches_if_needed (solver);
  return res;
}

void
kissat_inc_new_binary_clause (kissat * solver,
...
reference
kissat_inc_new_redundant_clause (kissat * solver, unsigned glue)
{
  const unsigned size = SIZE_STACK (solver->clause.lits);
  unsigned *lits = BEGIN_STACK (solver->clause.lits);
...
mark_clause_as_garbage (kissat * solver, clause * c)
{
  ...
  LOGCLS (c, "garbage");
  if (!c->redundant)
    kissat_inc_mark_removed_literals (solver, c->size, c->lits);
...
void
kissat_inc_mark_clause_as_garbage (kissat * solver, clause * c)
{
  assert (!c->garbage);
  mark_clause_as_garbage (solver, c);
  size_t bytes = kissat_inc_actual_bytes_of_clause (c);
  ADD (arena_garbage, bytes);
}

clause *
kissat_inc_delete_clause (kissat * solver, clause * c)
{
  LOGCLS (c, "delete");
  assert (c->size > 2);
  assert (c->garbage);
...
void
kissat_inc_delete_binary (kissat * solver,
		      bool redundant, bool hyper, unsigned a, unsigned b)
{
  LOGBINARY (a, b, "delete");
  if (!redundant)
    {
      kissat_inc_mark_removed_literal (solver, a);
      kissat_inc_mark_removed_literal (solver, b);
    }
  else if (hyper)
    DEC (hyper_binaries);
