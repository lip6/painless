...
mark_clause_as_used (kissat * solver, clause * c)
{
  ...
  if (c->hyper)
    return;
  const unsigned new_glue = kissat_inc_recompute_glue (solver, c);
  if (new_glue < c->glue)
    kissat_inc_promote_clause (solver, c, new_glue);
...
clause *
kissat_inc_deduce_first_uip_clause (kissat * solver, clause * conflict)
{
  START (deduce);
  ...
  for (;;)
	  ...
	  solver->resolvent_size < solver->antecedent_size)
	{
	  ...
	  clause *reason = kissat_inc_dereference_clause (solver, a->reason);
	  assert (!reason->garbage);
	  clause *res = kissat_inc_on_the_fly_strengthen (solver, reason, uip);
	  if (resolved == 1 && solver->resolvent_size < conflict_size)
	    {
	      assert (!conflict->garbage);
	      assert (conflict_size > 2);
	      kissat_inc_on_the_fly_subsume (solver, res, conflict);
	    }
	  STOP (deduce);
	  return res;
	}
