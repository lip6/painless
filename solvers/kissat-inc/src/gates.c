...
size_t
kissat_inc_mark_binaries (kissat * solver, unsigned lit)
{
...
void
kissat_inc_unmark_binaries (kissat * solver, unsigned lit)
{
  value *marks = solver->marks;
  watches *watches = &WATCHES (lit);
  for (all_binary_large_watches (watch, *watches))
...
bool
kissat_inc_find_gates (kissat * solver, unsigned lit)
{
  ...
  if (!WATCHES (not_lit).size)
    return false;
  if (kissat_inc_find_equivalence_gate (solver, lit))
    return true;
  if (kissat_inc_find_and_gate (solver, lit, 0))
    return true;
  if (kissat_inc_find_and_gate (solver, not_lit, 1))
    return true;
  if (kissat_inc_find_if_then_else_gate (solver, lit, 0))
    return true;
  if (kissat_inc_find_if_then_else_gate (solver, not_lit, 1))
    return true;
  if (kissat_inc_find_xor_gate (solver, lit, 0))
    return true;
  if (kissat_inc_find_xor_gate (solver, not_lit, 1))
    return true;
  return false;
}
...
get_antecedents (kissat * solver, unsigned lit, unsigned negative)
{
  ...
  LOG ("got %zu antecedent %.0f%% and %zu gate clauses %.0f%% "
       ...
       size_antecedents, kissat_inc_percent (size_antecedents, watches->size),
       size_gates, kissat_inc_percent (size_gates, watches->size),
       watches->size, LOGLIT (lit));
#endif
}

void
kissat_inc_get_antecedents (kissat * solver, unsigned lit)
{
