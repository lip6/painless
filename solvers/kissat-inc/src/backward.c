	  ...
	  if (found)
	    {
	      ...
	      INC (duplicated);
	      kissat_inc_disconnect_binary (solver, other, min_lit);
	      kissat_inc_delete_binary (solver, false, false, min_lit, other);
	      kissat_inc_update_after_removing_variable (solver, IDX (other));
	      q--;
	    }
	  else
	    found = true;
      ...
      else
	{
	  ...
	  for (all_literals_in_clause (lit, c))
	      ...
	      if (value > 0)
		{
		  ...
		  kissat_inc_update_after_removing_clause (solver, c,
						       INVALID_LIT);
		  break;
		}
	      if (value < 0)
	  ...
	  if (remove == INVALID_LIT)
	    {
	      ...
	      kissat_inc_mark_clause_as_garbage (solver, c);
	      kissat_inc_update_after_removing_clause (solver, c, INVALID_LIT);
	      q--;
	      continue;
	    }
	  ...
	  if (old_size > 3)
	      ...
	      for (unsigned i = 0; i < c->size; i++)
		{
		  ...
		  assert (!value);
		  lits[new_size++] = lit;
		  kissat_inc_mark_added_literal (solver, remove);
		}
	      ...
	      if (!c->shrunken)
		{
	      ...
	      kissat_inc_disconnect_reference (solver, remove, ref);
	    }
	  else
	    {
	      assert (old_size == 3);
	  ...
	  kissat_inc_update_after_removing_variable (solver, IDX (remove));
	}
    }
  ...
  if (unit)
    {
      ...
      kissat_inc_assign_unit (solver, min_lit);
  ...
  for (all_stack (unsigned, other, solver->delayed))
    {
      ...
      kissat_inc_watch_other (solver, false, false, other, min_lit);
      kissat_inc_watch_other (solver, false, false, min_lit, other);
      kissat_inc_mark_added_literal (solver, other);
      kissat_inc_mark_added_literal (solver, min_lit);
    }
...
bool
kissat_inc_backward_subsume_temporary (kissat * solver, reference ignore)
{
