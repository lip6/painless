  ...
  for (;;)
	...
	START:
	  ...
	  if (ch == '\n')
		...
		goto COMPLETE;
	      arg *= sign;
	      const opt *opt = kissat_inc_options_has (name);
	      if (opt)
		{
		  (void) kissat_inc_options_set_opt (&solver->options, opt, arg);
		  kissat_inc_verbose (solver,
				  "parsed embedded option '--%s=%d'",
				  name, arg);
		}
	      else
		kissat_inc_warning (solver,
...
// 		  "parsed 'p cnf %d %" PRIu64 "' header", variables, clauses);
  *max_var_ptr = variables;
  kissat_inc_reserve (solver, variables);
  uint64_t parsed = 0;
  int lit = 0;
  for (;;)
    {
...
const char *
kissat_inc_parse_dimacs (kissat * solver,
		     strictness strict,
		     file * file, uint64_t * lineno_ptr, int *max_var_ptr)
{
  const char *res;
