...
push_witness_literal (kissat * solver, unsigned ilit)
{
  ...
  const extension ext = kissat_inc_extension (true, elit);
...
push_clause_literal (kissat * solver, unsigned ilit)
{
  ...
  if (value < 0)
  ...
  else
    {
      ...
      const extension ext = kissat_inc_extension (false, elit);
      PUSH_STACK (solver->extend, ext);
    }
}
...
void
kissat_inc_weaken_clause (kissat * solver, unsigned lit, clause * c)
{
  INC (weakened);
...
void
kissat_inc_weaken_binary (kissat * solver, unsigned lit, unsigned other)
{
  INC (weakened);
...
void
kissat_inc_weaken_unit (kissat * solver, unsigned lit)
{
  INC (weakened);
