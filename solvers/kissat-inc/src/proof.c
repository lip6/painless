...
void
kissat_inc_init_proof (kissat * solver, file * file, bool binary)
{
  assert (file);
  assert (!solver->proof);
  proof *proof = kissat_inc_calloc (solver, 1, sizeof (struct proof));
  proof->binary = binary;
  proof->file = file;
  solver->proof = proof;
...
void
kissat_inc_release_proof (kissat * solver)
{
  proof *proof = solver->proof;
  assert (proof);
  LOG ("stopping to trace proof");
  RELEASE_STACK (proof->line);
  kissat_inc_free (solver, proof, sizeof (struct proof));
  solver->proof = 0;
}
...
#define PERCENT_LINES(NAME) \
  kissat_inc_percent (proof->NAME, proof->lines)

void
kissat_inc_print_proof_statistics (kissat * solver, bool verbose)
{
  proof *proof = solver->proof;
  ...
  if (verbose)
    PRINT_STAT ("proof_literals", proof->literals,
		kissat_inc_average (proof->literals, proof->lines),
		"", "per line");
}
...
print_binary_proof_line (proof * proof)
{
  ...
  for (all_stack (int, elit, proof->line))
	...
	{
	  ch = (x & 0x7f) | 0x80;
	  kissat_inc_putc (proof->file, ch);
	  x >>= 7;
	}
      kissat_inc_putc (proof->file, (unsigned char) x);
    }
  kissat_inc_putc (proof->file, 0);
}
...
void
kissat_inc_add_binary_to_proof (kissat * solver, unsigned a, unsigned b)
{
  proof *proof = solver->proof;
  assert (proof);
...
void
kissat_inc_add_clause_to_proof (kissat * solver, clause * c)
{
  proof *proof = solver->proof;
  assert (proof);
...
void
kissat_inc_add_empty_to_proof (kissat * solver)
{
  proof *proof = solver->proof;
  assert (proof);
  assert (EMPTY_STACK (proof->line));
...
void
kissat_inc_add_lits_to_proof (kissat * solver, size_t size, unsigned *ilits)
{
  proof *proof = solver->proof;
  assert (proof);
...
void
kissat_inc_add_unit_to_proof (kissat * solver, unsigned ilit)
{
  proof *proof = solver->proof;
  assert (proof);
  assert (EMPTY_STACK (proof->line));
...
void
kissat_inc_shrink_clause_in_proof (kissat * solver, clause * c,
...
void
kissat_inc_delete_binary_from_proof (kissat * solver, unsigned a, unsigned b)
{
  proof *proof = solver->proof;
  assert (proof);
  import_internal_proof_binary (solver, proof, a, b);
...
void
kissat_inc_delete_clause_from_proof (kissat * solver, clause * c)
{
  proof *proof = solver->proof;
  assert (proof);
  import_proof_clause (solver, proof, c);
...
void
kissat_inc_delete_external_from_proof (kissat * solver, size_t size, int *elits)
{
  proof *proof = solver->proof;
  assert (proof);
...
void
kissat_inc_delete_internal_from_proof (kissat * solver,
				   size_t size, unsigned *ilits)
{
  proof *proof = solver->proof;
  assert (proof);
...
#else
int kissat_inc_proof_dummy_to_avoid_warning;
#endif
