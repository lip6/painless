...
{
  const unsigned assigned = kissat_inc_assigned (solver);
...
clause *
kissat_inc_search_propagate (kissat * solver)
{
  ...
  if(solver->stable && solver->heuristic==1){
      ...
      while(i>=0 && LEVEL(lit)==solver->level){
	    lit = PEEK_STACK (solver->trail, i);
            kissat_inc_bump_chb(solver,IDX(lit), conflict? 1.0 : 0.9); 
	    i--;	    
      }
  }  
  if(solver->stable && solver->heuristic==1 && conflict) kissat_inc_decay_chb(solver);
