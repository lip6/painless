...
report_resized (kissat * solver, const char *mode, arena before)
{
  ...
  kissat_inc_phase (solver, "arena", GET (arena_resized),
...
reference
kissat_inc_allocate_clause (kissat * solver, size_t size)
{
  assert (size <= UINT_MAX);
  const size_t res = SIZE_STACK (solver->arena);
  assert (res <= MAX_REF);
  const size_t bytes = kissat_inc_bytes_of_clause (size);
  assert (kissat_inc_aligned_word (bytes));
  const size_t needed = bytes / sizeof (word);
  assert (needed <= UINT_MAX);
  size_t capacity = CAPACITY_STACK (solver->arena);
  assert (kissat_inc_is_power_of_two (MAX_ARENA));
  ...
  if (needed > available)
      ...
      do
	{
	  ...
	  if (capacity == MAX_ARENA)
	    kissat_inc_fatal ("maximum arena capacity "
			  "of 2^%d words %s exhausted",
			  LD_MAX_ARENA,
			  FORMAT_BYTES (MAX_ARENA * sizeof (word)));
	  kissat_inc_stack_enlarge (solver, (chars *) & solver->arena,
				sizeof (word));
...
void
kissat_inc_shrink_arena (kissat * solver)
{
  const arena before = solver->arena;
  ...
  kissat_inc_phase (solver, "arena", GET (arena_resized),
		...
		kissat_inc_percent (size, capacity),
...
bool
kissat_inc_clause_in_arena (const kissat * solver, const clause * c)
{
  if (!kissat_inc_aligned_pointer (c))
    return false;
  const char *p = (char *) c;
  ...
  if (p < begin)
    return false;
  const size_t bytes = kissat_inc_bytes_of_clause (c->size);
  if (end < p + bytes)
    return false;
  return true;
}
