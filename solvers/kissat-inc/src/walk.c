		      ...
		      unsigned counter_ref, unsigned *size_ptr)
{
  ...
  if (tagged.binary)
  ...
  else
    {
      ...
      clause *c = kissat_inc_dereference_clause (solver, clause_ref);
      *size_ptr = c->size;
      lits = c->lits;
    }
  return lits;
...
init_score_table (walker * walker)
{
  ...
  walker->table = kissat_inc_malloc (solver, exponents * sizeof (double));
  ...
  kissat_inc_phase (solver, "walk", GET (walks),
...
import_decision_phases (walker * walker)
{
  ...
  kissat_inc_phase (solver, "walk", GET (walks), "copied decision phases");
}
...
connect_binary_counters (walker * walker)
{
  ...
  for (unsigned binary_ref = 0; binary_ref < size; binary_ref++)
	...
	continue;
      ...
      kissat_inc_push_large_watch (solver, first_watches, counter_ref);
      kissat_inc_push_large_watch (solver, second_watches, counter_ref);
  ...
  kissat_inc_phase (solver, "walk", GET (walks),
		...
		unsat, kissat_inc_percent (unsat, counter_ref), counter_ref);
#ifdef QUIET
  (void) unsat;
#endif
  return counter_ref;
...
connect_large_counters (walker * walker, unsigned counter_ref)
{
  ...
  for (all_clauses (c))
      ...
      for (all_literals_in_clause (lit, c))
	{
	  ...
	  kissat_inc_push_large_watch (solver, watches, counter_ref);
	  if (value < 0)
	    continue;
	  count++;
	}
  ...
  kissat_inc_phase (solver, "walk", GET (walks),
		...
		unsat, kissat_inc_percent (unsat, large), large);
#ifdef QUIET
  (void) large;
  (void) unsat;
#endif
...
report_initial_minimum (kissat * solver, walker * walker)
{
  ...
  kissat_inc_very_verbose (solver, "initial minimum of %u unsatisfied clauses",
		       walker->minimum);
}
...
init_walker (kissat * solver, walker * walker, litpairs * binaries)
{
  ...
  solver->values = kissat_inc_calloc (solver, LITS, 1);

  import_decision_phases (walker);

  walker->counters = kissat_inc_malloc (solver, clauses * sizeof (counter));
  walker->refs = kissat_inc_malloc (solver, clauses * sizeof (tagged));
  ...
  kissat_inc_phase (solver, "walk", GET (walks),
		...
		kissat_inc_percent (walker->initial, IRREDUNDANT_CLAUSES),
		IRREDUNDANT_CLAUSES);
...
{
  kissat *solver = walker->solver;
  kissat_inc_dealloc (solver, walker->table, walker->exponents, sizeof (double));
  unsigned clauses = walker->clauses;
  kissat_inc_dealloc (solver, walker->refs, clauses, sizeof (tagged));
  kissat_inc_dealloc (solver, walker->counters, clauses, sizeof (counter));
  RELEASE_STACK (walker->unsat);
  RELEASE_STACK (walker->scores);
  RELEASE_STACK (walker->trail);
  kissat_inc_free (solver, solver->values, LITS);
  RELEASE_STACK (walker->unsat);
...
pick_literal (kissat * solver, walker * walker)
{
  assert (walker->current == SIZE_STACK (walker->unsat));
  const unsigned pos =
    kissat_inc_next_random32 (&walker->random) % walker->current;
  ...
  const double random = kissat_inc_pick_double (&walker->random);
...
save_walker_trail (kissat * solver, walker * walker, bool keep)
{
  ...
  LOG ("flushed %u literals %.0f%% from trail",
       walker->best, kissat_inc_percent (walker->best, size_trail));
  ...
  LOG ("keeping %u literals %.0f%% on trail",
       kept, kissat_inc_percent (kept, size_trail));
  LOG ("reset best trail position to 0");
  walker->best = 0;
}
...
update_best (kissat * solver, walker * walker)
{
  ...
  walker->minimum = walker->current;
#ifndef QUIET
  int verbosity = kissat_inc_verbosity (solver);
...
local_search_round (walker * walker, unsigned round)
{
  ...
  kissat_inc_very_verbose (solver,
    ...
    walker->flipped, steps, kissat_inc_average (steps, walker->flipped));
  // *INDENT-ON*
...
walking_phase (kissat * solver)
{
  ...
  kissat_inc_enter_dense_mode (solver, &irredundant, &redundant);
  walker walker;
  ...
  kissat_inc_resume_sparse_mode (solver, false, &irredundant, &redundant);
  ...
  if (last_irredundant > MAX_WALK_REF)
    {
      kissat_inc_phase (solver, "walk", GET (walks),
		    ...
		    last_irredundant);
      return kissat_inc_rephase_best (solver);
    }
...
int
kissat_inc_walk_initially (kissat * solver)
{
