...
do { \
  assert (FULL_STACK (S)); \
  kissat_inc_stack_enlarge (solver, (chars*) &(S), sizeof *(S).begin); \
} while (0)

#define SHRINK_STACK(S) \
do { \
  if (!FULL_STACK (S)) \
    kissat_inc_shrink_stack (solver, (chars*) &(S), sizeof *(S).begin); \
} while (0)
...
do { \
  kissat_inc_dealloc (solver, (S).begin, CAPACITY_STACK (S), sizeof *(S).begin); \
  INIT_STACK (S); \
} while (0)
...
void kissat_inc_stack_enlarge (struct kissat *, chars *, size_t size_of_element);
void kissat_inc_shrink_stack (struct kissat *, chars *, size_t size_of_element);
