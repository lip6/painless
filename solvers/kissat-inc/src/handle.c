...
const char *
kissat_inc_signal_name (int sig)
{
#define SIGNAL(SIG) \
...
void
kissat_inc_reset_signal_handler (void)
{
  if (!handler_set)
    return;
#define SIGNAL(SIG) \
...
void
kissat_inc_init_signal_handler (void (*h) (int sig))
{
  assert (!handler);
  handler = h;
  handler_set = true;
...
void
kissat_inc_init_alarm (void (*handler) (void))
{
  assert (handler);
  assert (!caught_alarm);
