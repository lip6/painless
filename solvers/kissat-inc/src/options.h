...
extern const opt *kissat_inc_options_begin;
extern const opt *kissat_inc_options_end;

#define all_options(O) \
  const opt * O = kissat_inc_options_begin; O != kissat_inc_options_end; ++O

const char *kissat_inc_parse_option_name (const char *arg, const char *name);
bool kissat_inc_parse_option_value (const char *val_str, int *res_ptr);

#ifndef NOPTIONS

void kissat_inc_options_usage (void);

const opt *kissat_inc_options_has (const char *name);

#define kissat_inc_options_max_name_buffer_size 32

bool kissat_inc_options_parse_arg (const char *arg, char *name, int *val_str);
void kissat_inc_options_print_value (int value, char *buffer);
...
void kissat_inc_init_options (options *);

int kissat_inc_options_get (const options *, const char *name);
int kissat_inc_options_set_opt (options *, const opt *, int new_value);
int kissat_inc_options_set (options *, const char *name, int new_value);

void kissat_inc_print_embedded_option_list (void);
void kissat_inc_print_option_range_list (void);

static inline int *
kissat_inc_options_ref (const options * options, const opt * o)
{
  if (!o)
    return 0;
  assert (kissat_inc_options_begin <= o);
  assert (o < kissat_inc_options_end);
  return (int *) options + (o - kissat_inc_options_begin);
}
...
void kissat_inc_init_options (void);
int kissat_inc_options_get (const char *name);

#define GET_OPTION(N) kissat_inc_options_ ## N
