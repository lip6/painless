...
find_forward_subsumption_candidates (kissat * solver, references * candidates)
{
  ...
  for (all_clauses (c))
	...
	left_over_from_last_subsumption_round++;
      const unsigned ref = kissat_inc_reference_clause (solver, c);
  ...
  for (all_stack (reference, ref, *candidates))
    kissat_inc_dereference_clause (solver, ref)->subsume = true;
}
      ...
      else
	{
	  ...
	  for (all_literals_in_clause (other, d))
	      ...
	      if (value > 0)
		{
		  ...
		  kissat_inc_mark_clause_as_garbage (solver, d);
		  assert (d->garbage);
...
forward_subsumed_clause (kissat * solver, clause * c, bool * removed)
{
  ...
  if (subsume)
      ...
      else
	{
	  ...
	  kissat_inc_mark_removed_literal (solver, remove);
	  if (non_false > 3)
	    {
	  ...
	  else
	    {
	      ...
	      assert (!c->garbage);
	      assert (!c->hyper);
	      const size_t bytes = kissat_inc_actual_bytes_of_clause (c);
	      ...
	      for (all_literals_in_clause (lit, c))
		{
	      ...
	      kissat_inc_watch_other (solver, false, false, first, second);
	      kissat_inc_watch_other (solver, false, false, second, first);
	      *removed = true;
	    }
	}
    }
...
connect_subsuming (kissat * solver, unsigned occlim, clause * c)
{
  ...
  kissat_inc_connect_literal (solver, min_lit, ref);
}
...
forward_subsume_all_clauses (kissat * solver)
{
  ...
  kissat_inc_phase (solver, "forward", GET (forward_subsumptions),
		"scheduled %zu irredundant clauses %.0f%%", scheduled,
		kissat_inc_percent (scheduled,
...

  {
    SET_EFFICIENCY_BOUND (limit, subsume, subsumption_checks,
			  search_ticks, kissat_inc_nlogn (scheduled));
  ...
  if (subsumed)
    kissat_inc_phase (solver, "forward", GET (forward_subsumptions),
		  ...
		  subsumed, kissat_inc_percent (subsumed, checked),
		  checked, kissat_inc_percent (checked, scheduled));
  if (strengthened)
    kissat_inc_phase (solver, "forward", GET (forward_subsumptions),
		  "strengthened %zu clauses %.2f%% of %zu checked %.0f%%",
		  strengthened, kissat_inc_percent (strengthened, checked),
		  checked, kissat_inc_percent (checked, scheduled));
  ...
  struct flags *saved = kissat_inc_calloc (solver, solver->vars, sizeof *saved);
  ...
  kissat_inc_free (solver, saved, solver->vars * sizeof *saved);
...
void
kissat_inc_forward_subsume_during_elimination (kissat * solver)
{
  START (forward);
...
bool
kissat_inc_forward_subsume_temporary (kissat * solver)
{
