...
schedule_probes (kissat * solver, unsigned round, unsigneds * roots)
{
  ...
  (void) round;
#else
  kissat_inc_phase (solver, "failed", GET (failed_literal_probings),
		"round %u: scheduled %zu probes %.0f%% out of all variables",
		round, res, kissat_inc_percent (res, solver->active));
  if (scheduled[1])
    kissat_inc_phase (solver, "failed", GET (failed_literal_probings),
		  "round %u: prioritized %u probes %.0f%% of all scheduled",
		  round, scheduled[1], kissat_inc_percent (scheduled[1], res));
#endif
  return res;
}
		   ...
		   unsigned a, unsigned b)
{
  ...
  if (p && !q)
    return false;
  const double s = kissat_inc_get_heap_score (scores, i);
  const double t = kissat_inc_get_heap_score (scores, j);
  if (s < t)
    return true;
  if (s > t)
    return false;
	     ...
	     unsigned *failed_ptr, uint64_t * resolved_ptr)
{
...
#endif
  unsigned *stamps = kissat_inc_calloc (solver, LITS, sizeof *stamps);
  const value *values = solver->values;
  flags *flags = solver->flags;
  ...
  while (!EMPTY_STACK (*probes))
	...
	LOG ("probing %s[%g]", LOGLIT (probe),
	     kissat_inc_get_heap_score (solver->heuristic==0?&solver->scores:&solver->scores_chb, IDX (probe)));
      else
	LOG ("probing %s{%u}", LOGLIT (probe), LINK (IDX (probe)).stamp);
      probed++;
      const unsigned propagated = solver->propagated;
      kissat_inc_internal_assume (solver, probe);
      assert (solver->level == 1);
      assert (values[probe] > 0);
      clause *conflict = kissat_inc_hyper_propagate (solver, 0);
      if (conflict)
	{
	  ...
	  conflict = kissat_inc_probing_propagate (solver, 0);
	  if (conflict)
	    {
	      (void) kissat_inc_analyze (solver, conflict);
	...
	if (flags[IDX (lit)].probe)
      ...
      if (!prioritized)
	{
	  kissat_inc_phase (solver, "failed",
  ...
  kissat_inc_dealloc (solver, stamps, LITS, sizeof *stamps);
  ...
  kissat_inc_phase (solver, "failed",
		...
		round,
		failed, kissat_inc_percent (failed, probed),
		probed, kissat_inc_percent (probed, scheduled));

  kissat_inc_phase (solver, "failed", GET (failed_literal_probings),
		"round %u: %" PRIu64 " hyper binary resolvents "
		"%.1f per probe", round, redundant,
		kissat_inc_average (redundant, probed));
#else
  (void) round;
#endif
...
void
kissat_inc_failed_literal_probing (kissat * solver)
{
  if (solver->inconsistent)
    return;
  ...
  SET_EFFICIENCY_BOUND (limit, failed, probing_ticks,
			search_ticks, kissat_inc_nlogn (solver->active));
