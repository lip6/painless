...
#define MB \
  (kissat_inc_current_resident_set_size ()/(double)(1<<20))

#define REMAINING_VARIABLES \
  kissat_inc_percent (solver->active, SIZE_STACK(solver->import))

#define REPORTS \
REP("seconds", "5.2f", kissat_inc_time (solver)) \
REP("MB", "2.0f", MB) \
...
void
kissat_inc_report (kissat * solver, bool verbose, char type)
{
...
int kissat_inc_report_dummy_to_avoid_warning;
