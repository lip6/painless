...
assign_and_propagate_units (kissat * solver, unsigneds * units)
{
  ...
  while (!solver->inconsistent && !EMPTY_STACK (*units))
      ...
      if (!value)
	{
	  ...
	  INC (failed);
	  if (!kissat_inc_probing_propagate (solver, 0))
	    continue;
...
determine_representatives (kissat * solver, unsigned *repr)
{
  ...
  unsigned *mark = kissat_inc_calloc (solver, LITS, sizeof *mark);
  unsigned *reach = kissat_inc_malloc (solver, LITS * sizeof *reach);
	  ...
	  if (lit == INVALID_LIT)
	    {
  		...
  		unsigned ticks = kissat_inc_cache_lines (watches->size, sizeof (watch));
  ...
  RELEASE_STACK (units);
  kissat_inc_free (solver, reach, bytes);
  kissat_inc_free (solver, mark, bytes);
...
add_representative_equivalences (kissat * solver, unsigned *repr)
{
  if (solver->inconsistent)
    return 0;
  bool *eliminate = kissat_inc_calloc (solver, VARS, sizeof *eliminate);
      ...
      for (all_variables (idx))
	{
	  ...
	  if (incremental || other_value)
		...
		kissat_inc_weaken_binary (solver, not_lit, other);
	      if (other_value >= 0)
		kissat_inc_weaken_binary (solver, lit, not_other);
	    }
	  else
	    {
	      kissat_inc_weaken_binary (solver, not_lit, other);
	      kissat_inc_weaken_unit (solver, lit);
	    }
	}
    }
  if (eliminate)
    kissat_inc_dealloc (solver, eliminate, VARS, sizeof *eliminate);
}
      ...
      else if (substitute || shrink)
	{
	  ...
	  if (!size)
	  ...
	  else if (size == 2)
	    {
	      ...
	      kissat_inc_new_binary_clause (solver, redundant, first, second);
	      kissat_inc_mark_clause_as_garbage (solver, c);
	    }
	  else
	    {
...
substitute_round (kissat * solver, unsigned round)
{
  ...
  size_t bytes = LITS * sizeof (unsigned);
  unsigned *repr = kissat_inc_malloc (solver, bytes);
  ...
  kissat_inc_dealloc (solver, repr, LITS, sizeof *repr);
  unsigned removed = active - solver->active;
  kissat_inc_phase (solver, "substitute", GET (substitutions),
		"round %u removed %u variables %.0f%%",
		round, removed, kissat_inc_percent (removed, active));
...
void
kissat_inc_substitute (kissat * solver, bool first)
{
  if (solver->inconsistent)
    return;
