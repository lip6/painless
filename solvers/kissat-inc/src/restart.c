...
bool
kissat_inc_restarting (kissat * solver)
{
  ...
  if (CONFLICTS < solver->limits.restart.conflicts)
    return false;
  kissat_inc_switch_search_mode (solver);
  if (solver->stable)
    return kissat_inc_reluctant_triggered (&solver->reluctant);
...
void
kissat_inc_new_focused_restart_limit (kissat * solver)
{
  assert (!solver->stable);
  limits *limits = &solver->limits;
  uint64_t delta = GET_OPTION (restartint) - 1;
  delta += kissat_inc_logn (solver->statistics.restarts);
  limits->restart.conflicts = CONFLICTS + delta;
  kissat_inc_extremely_verbose (solver, "next focused restart scheduled at %"
...
{
  const unsigned next_idx = kissat_inc_next_decision_variable (solver);
  const heap *scores = solver->heuristic==0?&solver->scores:&solver->scores_chb;
  const unsigned next_idx_score = kissat_inc_get_heap_score (scores, next_idx);
  ...
  while (res < solver->level)
	...
	kissat_inc_get_heap_score (scores, decision_idx);
...
void kissat_inc_restart_mab(kissat * solver){   
...
void
kissat_inc_restart (kissat * solver)
{
  START (restart);  
  INC (restarts);
  ...
  if (solver->stable && solver->mab) 
     kissat_inc_restart_mab(solver);
  unsigned new_heuristic = solver->heuristic;

  unsigned level = old_heuristic==new_heuristic?reuse_trail (solver):0;

  kissat_inc_extremely_verbose (solver,
  ...
  kissat_inc_backtrack (solver, level);
  if (solver->stable && solver->mab) solver->heuristic = new_heuristic;

  if (!solver->stable)
    kissat_inc_new_focused_restart_limit (solver);

  if (solver->stable && solver->mab && old_heuristic!=new_heuristic) kissat_inc_update_scores(solver);
