...
#ifndef NDEBUG
  clause *new_next = kissat_inc_next_clause (c);
...
binary_on_the_fly_strengthen (kissat * solver, clause * c, unsigned lit)
{
  ...
  const reference ref = kissat_inc_reference_clause (solver, c);
  kissat_inc_unwatch_blocking (solver, c->lits[0], ref);
  kissat_inc_unwatch_blocking (solver, c->lits[1], ref);
  kissat_inc_mark_clause_as_garbage (solver, c);
  clause *conflict =
    kissat_inc_binary_conflict (solver, redundant, first, second);
  INC (conflicts);
  return conflict;
}

clause *
kissat_inc_on_the_fly_strengthen (kissat * solver, clause * c, unsigned lit)
{
  assert (!c->garbage);
...
void
kissat_inc_on_the_fly_subsume (kissat * solver, clause * c, clause * d)
{
