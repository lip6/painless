...
void
kissat_inc_release_heap (kissat * solver, heap * heap)
{
...
void
kissat_inc_check_heap (heap * heap)
{
...
void
kissat_inc_resize_heap (kissat * solver, heap * heap, unsigned new_size)
{
  ...
  heap->pos = kissat_inc_nrealloc (solver, heap->pos,
			       old_size, new_size, sizeof (unsigned));
  if (heap->tainted)
    {
      heap->score = kissat_inc_nrealloc (solver, heap->score,
	...
	DEALLOC (heap->score, old_size);
      heap->score = kissat_inc_calloc (solver, new_size, sizeof (double));
    }
  heap->size = new_size;
#ifdef CHECK_HEAP
  kissat_inc_check_heap (heap);
#endif
}
...
void
kissat_inc_push_heap (kissat * solver, heap * heap, unsigned idx)
{
  LOG ("push heap %u", idx);
  assert (!kissat_inc_heap_contains (heap, idx));
  IMPORT (idx);
...
void
kissat_inc_pop_heap (kissat * solver, heap * heap, unsigned idx)
{
  LOG ("pop heap %u", idx);
  assert (kissat_inc_heap_contains (heap, idx));
  IMPORT (idx);
...
void
kissat_inc_update_heap (kissat * solver, heap * heap,
		    unsigned idx, double new_score)
{
  const double old_score = kissat_inc_get_heap_score (heap, idx);
...
double
kissat_inc_max_score_on_heap (heap * heap)
{
  if (!heap->tainted)
    return 0;
  assert (heap->vars);
...
void
kissat_inc_rescale_heap (kissat * solver, heap * heap, double factor)
{
...
void
kissat_inc_dump_heap (heap * heap)
{
  dump_heap (heap);
}
