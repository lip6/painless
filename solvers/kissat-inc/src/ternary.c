...
connect_ternary_clauses (kissat * solver, value * ternary)
{
  ...
  for (all_clauses (c))
	...
	continue;
      ...
      kissat_inc_connect_clause (solver, c);
      connected++;
    }
}
...
schedule_ternary (kissat * solver, value * ternary)
{
  ...
  for (all_variables (idx))
	...
	continue;
      if (!schedule->size)
	kissat_inc_resize_heap (solver, schedule, solver->vars);
  ...
  kissat_inc_phase (solver, "ternary", GET (hyper_ternary_phases),
		"scheduled %u variables %.0f%%",
		scheduled, kissat_inc_percent (scheduled, solver->active));
		    ...
		    unsigned lit, clause * c, clause * d)
{
  ...
  if (size == 3)
  ...
  else
    {
      ...
      const reference c_ref = kissat_inc_reference_clause (solver, c);
      const reference d_ref = kissat_inc_reference_clause (solver, d);
	     ...
	     unsigned first, unsigned second)
{
  ...
  const unsigned steps = kissat_inc_cache_lines (watches->size, sizeof (watch));
	      ...
	      unsigned first, unsigned second, unsigned third)
{
  ...
  for (all_binary_large_watches (watch, *watches))
      ...
      else
	{
	  ...
	  assert (kissat_inc_clause_in_arena (solver, d));
	  assert (d->size == 3);
				 ...
				 bool reschedule, unsigned lit)
{
  ...
  kissat_inc_update_variable_score (solver, schedule, idx);
  if (reschedule && !kissat_inc_heap_contains (schedule, idx))
    kissat_inc_push_heap (solver, schedule, idx);
}
...
add_ternary_resolvents (kissat * solver, tags * tags, uint64_t * resolved_ptr)
{
  ...
  while (!EMPTY_STACK (*tags))
	...
	{
	  (void) kissat_inc_new_binary_clause (solver, true, first, second);
	  assert (arena == BEGIN_STACK (solver->arena));
	}
      else
	{
	  reference ref = kissat_inc_new_redundant_clause (solver, 2);
...
#ifndef QUIET
  size_t scheduled = kissat_inc_size_heap (schedule);
#endif
  uint64_t resolved = 0;

  while (!kissat_inc_empty_heap (schedule))
    {
      if (TERMINATED (14))
	break;
      ...
      if (resolved > resolved_limit)
	{
      ...
      const unsigned idx = kissat_inc_max_heap (schedule);
      kissat_inc_pop_heap (solver, schedule, idx);
      assert (ACTIVE (idx));
...
#ifndef QUIET
  size_t remain = kissat_inc_size_heap (schedule);
  if (remain)
    kissat_inc_phase (solver, "ternary", GET (hyper_ternary_phases),
		  "remaining %u variables %.0f%% (incomplete ternary round)",
		  remain, kissat_inc_percent (remain, scheduled));
  else
    kissat_inc_phase (solver, "ternary", GET (hyper_ternary_phases),
...
void
kissat_inc_ternary (kissat * solver)
{
  ...
  kissat_inc_enter_dense_mode (solver, 0, 0);

  value *ternary = kissat_inc_calloc (solver, LITS, sizeof *ternary);
  ...
  if (scheduled)
    {
      SET_EFFICIENCY_BOUND (steps_limit, ternary,
			    ...
			    2 * CLAUSES + kissat_inc_nlogn (scheduled));

      resolved = ternary_round (solver, resolved_limit, steps_limit);
      kissat_inc_release_heap (solver, &solver->schedule);
    }

  kissat_inc_dealloc (solver, ternary, LITS, sizeof *ternary);
  kissat_inc_resume_sparse_mode (solver, false, 0, 0);
  ...
  kissat_inc_phase (solver, "ternary", GET (hyper_ternary_phases),
		...
		resolved,
		resolved3, kissat_inc_percent (resolved3, resolved),
		resolved2, kissat_inc_percent (resolved2, resolved));
#else
  kissat_inc_phase (solver, "ternary", GET (hyper_ternary_phases),
