...
void
kissat_inc_init_profiles (profiles * profiles)
{
...
print_profile (profile * p, double total)
{
  printf ("c %14.2f %7.2f %%  %s\n",
	  p->time, kissat_inc_percent (p->time, total), p->name);
}
...
void
kissat_inc_profiles_print (kissat * solver)
{
  profiles *named = &solver->profiles;
  double now = kissat_inc_process_time ();
...
void
kissat_inc_start (kissat * solver, profile * profile)
{
  const double now = kissat_inc_process_time ();
  push_profile (solver, profile, now);
}

void
kissat_inc_stop (kissat * solver, profile * profile)
{
...
void
kissat_inc_stop_search_and_start_simplifier (kissat * solver, profile * profile)
{
...
void
kissat_inc_stop_simplifier_and_resume_search (kissat * solver, profile * profile)
{
  struct profile *simplify = &PROFILE (simplify);
...
#else
int kissat_inc_profile_dummy_to_avoid_warning;
#endif
