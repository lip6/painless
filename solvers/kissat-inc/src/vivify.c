  ...
  if (non_false == 2)
    {
      ...
      kissat_inc_new_binary_clause (solver, c->redundant, first, second);
      kissat_inc_mark_clause_as_garbage (solver, c);
    }
  else
    {
      ...
      for (unsigned i = 0; i < old_size; i++)
	{
	  ...
	  const value value = kissat_inc_fixed (solver, lit);
	  assert (value <= 0);
	  if (value < 0)
	    continue;
	  lits[new_size++] = lit;
	...
	kissat_inc_promote_clause (solver, c, new_size - 1);
      if (!c->shrunken)
	{
	  c->shrunken = true;
				  ...
				  bool redundant, bool tier2)
{
  ...
  if (prioritized)
    {
      kissat_inc_phase (solver, mode, GET (probings),
		    "prioritized %zu %s clauses %.0f%%", prioritized,
		    type, kissat_inc_percent (prioritized, scheduled));
    }
  else
    {
      kissat_inc_phase (solver, mode, GET (probings),
...
{
  return kissat_inc_calloc (solver, LITS, sizeof (unsigned));
}

static inline bool
worse_candidate (kissat * solver, unsigned *counts, reference r, reference s)
{
  const clause *c = kissat_inc_dereference_clause (solver, r);
  const clause *d = kissat_inc_dereference_clause (solver, s);
  ...
  if (a->binary)
    {
      ...
      conflict = kissat_inc_binary_conflict (solver, a->redundant, lit, other);
    }
  else
    {
      const reference ref = a->reason;
      conflict = kissat_inc_dereference_clause (solver, ref);
    }
  a->analyzed = true;
		...
		clause * conflict, bool * irredundant_ptr)
{
  ...
  while (!subsumed && analyzed < SIZE_STACK (solver->analyzed))
      ...
      else
	{
	  ...
	  clause *reason = kissat_inc_dereference_clause (solver, ref);
	  if (reason->redundant)
	    irredundant = false;
	  subsumed = marks[lit];
	      ...
	      unsigned non_false, bool irredundant, unsigned implied)
{
  ...
  if (size == 1)
    ...
    {
      if (c->redundant)
	(void) kissat_inc_new_redundant_clause (solver, 1);
      else
	(void) kissat_inc_new_irredundant_clause (solver);
      kissat_inc_mark_clause_as_garbage (solver, c);
      vivify_inc_strengthened (solver, c);
      res = true;
    }
	...
	kissat_inc_promote_clause (solver, c, new_size - 1);
      ...
      kissat_inc_unwatch_blocking (solver, watched[0], ref);
      kissat_inc_unwatch_blocking (solver, watched[1], ref);
      kissat_inc_watch_blocking (solver, lits[0], lits[1], ref);
      kissat_inc_watch_blocking (solver, lits[1], lits[0], ref);
	       ...
	       unsigneds * sorted, unsigned *counts)
{
  ...
  if (unit == INVALID_LIT)
  ...
  else
    {
      ...
      kissat_inc_backtrack (solver, level - 1);
    }
      ...
      for (all_stack (unsigned, lit, *sorted))
	{
	  ...
	  if (counts)
	  ...
	  else
	    {
	      ...
	      if (solver->stable)
		printf ("[%g]", kissat_inc_get_heap_score (scores, idx));
	      else
  ...
  for (all_stack (unsigned, lit, *sorted))
      ...
      if (!value)
	{
	  ...
	  INC (vivify_assumed);
	  INC (vivify_probes);
	  kissat_inc_internal_assume (solver, not_lit);
	  if (solver->level == 1)
	    conflict = kissat_inc_hyper_propagate (solver, c);
	  else
	    conflict = kissat_inc_probing_propagate (solver, c);
	  if (!conflict)
	    continue;
...
vivify_round (kissat * solver, round round)
{
  ...
  unsigned *counts = 0;
  kissat_inc_flush_large_watches (solver);
  ...
  kissat_inc_watch_large_clauses (solver);
  ...
  kissat_inc_phase (solver, mode, GET (probings),
		...
		type, kissat_inc_percent (scheduled, total), total);
#endif
  SET_EFFICIENCY_BOUND (ticks_limit, vivify,
			probing_ticks, search_ticks,
			kissat_inc_nlogn (scheduled));
  ...
  if (solver->level)
    kissat_inc_backtrack (solver, 0);
  kissat_inc_dealloc (solver, counts, LITS, sizeof *counts);
  RELEASE_STACK (sorted);
#ifndef QUIET
  kissat_inc_phase (solver, mode, GET (probings),
		"vivified %zu %s clauses %.0f%% out of %zu tried",
		vivified, type, kissat_inc_percent (vivified, tried), tried);
  if (!solver->inconsistent)
    {
      ...
      if (remain)
	{
	  kissat_inc_phase (solver, mode, GET (probings),
			...
			remain, type, kissat_inc_percent (remain, scheduled),
			scheduled);
	  ...
	  if (prioritized)
	    kissat_inc_phase (solver, mode, GET (probings),
			  ...
			  prioritized, type,
			  kissat_inc_percent (prioritized, remain));
	  else
	    kissat_inc_phase (solver, mode, GET (probings),
...
void
kissat_inc_vivify (kissat * solver)
{
  if (solver->inconsistent)
    return;
  assert (!solver->level);
