...
bool
kissat_inc_reducing (kissat * solver)
{
...
collect_reducibles (kissat * solver, reducibles * reds, reference start_ref)
{
  ...
  while (start != end && (!start->redundant || start->keep))
    start = kissat_inc_next_clause (start);
  if (start == end)
    {
...
#endif
  for (clause * c = start; c != end; c = kissat_inc_next_clause (c))
    {
      if (!c->redundant)
	continue;
      if (c->garbage)
...
// *INDENT-OFF*
  ...
  if (flushed_hyper_ternary_clauses)
    kissat_inc_phase (solver, "reduced", GET (reductions),
      "reduced %zu unused hyper ternary clauses %.0f%% out of %zu",
      flushed_hyper_ternary_clauses,
	kissat_inc_percent (flushed_hyper_ternary_clauses,
...
mark_less_useful_clauses_as_garbage (kissat * solver, reducibles * reds)
{
  ...
  kissat_inc_phase (solver, "reduce",
		GET (reductions),
		"reducing %zu (%.0f%%) out of %zu (%.0f%%) "
		"reducible clauses",
		target, kissat_inc_percent (target, size),
		size, kissat_inc_percent (size, clauses));
#endif
  unsigned reduced = 0;
...
compacting (kissat * solver)
{
  ...
  LOG ("%u inactive variables %.0f%% <= limit %u %.0f%%",
       ...
       limit, kissat_inc_percent (limit, solver->vars));
  return compact;
}
...
int
kissat_inc_reduce (kissat * solver)
{
  START (reduce);
  INC (reductions);   LOG ("beginreduce");
  kissat_inc_phase (solver, "reduce", GET (reductions),
  ...
  if (start != INVALID_REF)
    {
      ...
      kissat_inc_phase (solver, "reduce", GET (reductions),
		    ...
		    kissat_inc_percent (words_to_sweep, arena_size)); 
#endif
      if (kissat_inc_flush_and_mark_reason_clauses (solver, start))
	{
	  reducibles reds;
	  INIT_STACK (reds);
	  if (collect_reducibles (solver, &reds, start))
	    {
	      ...
	      RELEASE_STACK (reds); 
	      kissat_inc_sparse_collect (solver, compact, start); 
	    }
	  else if (compact) 
	    kissat_inc_sparse_collect (solver, compact, start); 
	  else 
	    kissat_inc_unmark_reason_clauses (solver, start); 
	}
      else
	assert (solver->inconsistent);
    }
  else
    kissat_inc_phase (solver, "reduce", GET (reductions), "nothing to reduce");
