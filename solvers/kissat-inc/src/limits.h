...
changes kissat_inc_changes (struct kissat *);

bool kissat_inc_changed (changes before, changes after);

void kissat_inc_init_limits (struct kissat *);

uint64_t kissat_inc_scale_delta (struct kissat *, const char *, uint64_t);

uint64_t kissat_inc_scale_limit (struct kissat *,
			     const char *, uint64_t count, int base);

#define SCALE_LIMIT(COUNT,NAME) \
  kissat_inc_scale_limit (solver, #NAME, \
                      solver->statistics.COUNT, GET_OPTION (NAME))

uint64_t kissat_inc_logn (uint64_t);
uint64_t kissat_inc_ndivlogn (uint64_t);
uint64_t kissat_inc_linear (uint64_t);
uint64_t kissat_inc_nlogn (uint64_t);
uint64_t kissat_inc_nlognlogn (uint64_t);
uint64_t kissat_inc_quadratic (uint64_t);

#define NDIVLOGN(COUNT) kissat_inc_ndivlogn (COUNT)
#define LINEAR(COUNT) kissat_inc_linear (COUNT)
#define NLOGN(COUNT) kissat_inc_nlogn (COUNT)
#define NLOGNLOGN(COUNT) kissat_inc_nlognlogn (COUNT)
#define QUADRATIC(COUNT) kissat_inc_quadratic (COUNT)

#define INIT_CONFLICT_LIMIT(NAME,SCALE) \
do { \
  const uint64_t DELTA = GET_OPTION (NAME ## init); \
  const uint64_t SCALED = !(SCALE) ? DELTA : \
    kissat_inc_scale_delta (solver, #NAME, DELTA); \
...
do { \
  ...
  if (SUCCESS) \
    { \
      if (DELAY->current) \
	{ \
	  kissat_inc_very_verbose (solver, #NAME " delay reset"); \
