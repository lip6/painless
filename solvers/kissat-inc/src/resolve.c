...
bool
kissat_inc_generate_resolvents (kissat * solver, unsigned idx, unsigned *lit_ptr)
{
  ...
  const bool gates = kissat_inc_find_gates (solver, lit);
  kissat_inc_get_antecedents (solver, lit);
  ...
  if (failed)
    {
      ...
      if (update)
	kissat_inc_update_variable_score (solver, &solver->schedule, idx);
      return false;
    }
