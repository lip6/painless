...
do { \
  ...
  solver->NAME = \
    kissat_inc_nrealloc (solver, solver->NAME, old_size, new_size, \
...
do { \
  const size_t block_size = sizeof (TYPE); \
  TYPE *NAME = kissat_inc_calloc (solver, \
  ...
  kissat_inc_dealloc (solver, solver->NAME, \
...
void
kissat_inc_increase_size (kissat * solver, unsigned new_size)
{
  ...
  LOG ("%s before increasing size from %u to %u",
       FORMAT_BYTES (kissat_inc_allocated (solver)), old_size, new_size);
#endif
  ...
  if(solver->heuristic==0 || solver->mab)
     kissat_inc_resize_heap (solver, &solver->scores, new_size);
  if(solver->heuristic==1 || solver->mab)
     kissat_inc_resize_heap (solver, &solver->scores_chb, new_size);
...
void
kissat_inc_decrease_size (kissat * solver)
{
  const unsigned old_size = solver->size;
...
void
kissat_inc_enlarge_variables (kissat * solver, unsigned new_vars)
{
  ...
  if (old_size < new_vars)
	...
	{
	  if (kissat_inc_is_power_of_two (old_size))
	    {
      ...
      kissat_inc_increase_size (solver, new_size);
    }
