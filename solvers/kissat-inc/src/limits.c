...
changes
kissat_inc_changes (kissat * solver)
{
  changes res;
...
bool
kissat_inc_changed (changes b, changes a)
{
...
uint64_t
kissat_inc_logn (uint64_t count)
{
  return log10 (count + 10);
}

static uint64_t
kissat_inc_lognlogn (uint64_t count)
{
...
uint64_t
kissat_inc_ndivlogn (uint64_t count)
{
  return count / kissat_inc_logn (count);
}

uint64_t
kissat_inc_linear (uint64_t count)
{
  return count;
}

uint64_t
kissat_inc_nlogn (uint64_t count)
{
  return count * kissat_inc_logn (count);
}

uint64_t
kissat_inc_nlognlogn (uint64_t count)
{
  return count * kissat_inc_lognlogn (count);
}

uint64_t
kissat_inc_quadratic (uint64_t count)
{
  return count * count;
}

uint64_t
kissat_inc_scale_delta (kissat * solver, const char *pretty, uint64_t delta)
{
  const uint64_t C = IRREDUNDANT_CLAUSES;
  const double f = kissat_inc_logn (C);
  assert (f >= 1);
  const double ff = f * f;
  assert (ff >= 1);
...
uint64_t
kissat_inc_scale_limit (kissat * solver,
		    const char *pretty, uint64_t count, int base)
{
  assert (base >= 0);
  assert (count > 0);
  const double f = kissat_inc_logn (count - 1);
  assert (f >= 1);
...
init_enabled (kissat * solver)
{
  ...
  if (!GET_OPTION (simplify))
    ...
    probe = false;
  kissat_inc_very_verbose (solver, "probing %sabled", probe ? "en" : "dis");
  ...
  if (!GET_OPTION (simplify))
    ...
    eliminate = true;
  kissat_inc_very_verbose (solver, "eliminate %sabled", eliminate ? "en" : "dis");
  ...
  if (!GET_OPTION (simplify))
    ...
    autarky = true;
  kissat_inc_very_verbose (solver, "autarky %sabled", autarky ? "en" : "dis");
  ...
  if (GET_OPTION (stable) == 1)
    {
      ...
      kissat_inc_very_verbose (solver, "initial stable mode switching limit "
...
#endif
// *INDENT-OFF*
      solver->mode.entered = kissat_inc_process_time ();
      kissat_inc_very_verbose (solver,
  ...
  if (solver->enabled.eliminate)
    {
      ...
      kissat_inc_very_verbose (solver, "reset elimination bound to zero");
    }
}

void
kissat_inc_init_limits (kissat * solver)
{
  ...
  if (!solver->stable)
    kissat_inc_new_focused_restart_limit (solver);
