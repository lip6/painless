...
static inline void
kissat_inc_watch_large_delayed (kissat * solver, unsigneds * delayed)
{
  ...
  while (d != end_delayed)
    {
      ...
      LOGREF (ref, "watching %s blocking %s in", LOGLIT (lit),
	      LOGLIT (blocking));
      kissat_inc_push_blocking_watch (solver, lit_watches, blocking, ref);
    }
  CLEAR_STACK (*delayed);
}
...
static inline void
kissat_inc_update_probing_propagation_statistics (kissat * solver,
					      unsigned propagated)
{
...
static inline void
kissat_inc_delay_watching_large (kissat * solver, unsigneds * delayed,
			     unsigned lit, unsigned other, reference ref)
{
  const watch watch = kissat_inc_blocking_watch (other);
		   ...
		   const unsigned lit)
{
  ...
  uint64_t ticks = kissat_inc_cache_lines (watches->size, sizeof (watch));
  ...
  while (p != end_watches)
      ...
      if (head.type.binary)
	{
	  ...
	  if (blocking_value < 0)
	    {
	      res = kissat_inc_binary_conflict (solver, redundant,
					    not_lit, blocking);
	      break;
	    }
	  else
	    {
	      ...
	      kissat_inc_assign_binary (solver, values, assigned,
      ...
      else
	{
	  ...
	  if (other_value > 0)
	      ...
	      else if (!replacement_value)
		{
		  ...
		  assert (lits[0] != lits[1]);
		  *r = not_lit;
		  kissat_inc_delay_watching_large (solver, delayed,
		...
		{
		  assert (replacement_value < 0);
		  unsigned dom = kissat_inc_find_dominator (solver, other, c);
		  if (dom != INVALID_LIT)
		    {
		    ...
		    kissat_inc_assign_reference (solver, values,
...
#else
  kissat_inc_watch_large_delayed (solver, delayed);
#endif
