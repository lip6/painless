				...
				unsigned lit, unsigned ignore)
{
  ...
  unsigned ticks = 1 + kissat_inc_cache_lines (watches->size, sizeof (watch));
  ...
  for (all_binary_large_watches (watch, *watches))
      ...
      else
	{
	  ...
	  if (non_false == 1)
	    kissat_inc_assign_reference (solver, values, assigned, unit, ref, c);
	}
    }
...
bool
kissat_inc_dense_propagate (kissat * solver, unsigned limit, unsigned ignore_idx)
{
