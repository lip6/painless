...
unsigned *
kissat_inc_enlarge_vector (kissat * solver, vectors * vectors, vector * vector)
{
  ...
  assert (kissat_inc_is_power_of_two (MAX_VECTORS));
  ...
  if (new_vector_size > available)
    {
      ...
      do
	{
	  assert (kissat_inc_is_zero_or_power_of_two (capacity));

	  if (capacity == MAX_VECTORS)
	    kissat_inc_fatal ("maximum vector stack size "
			  "of 2^%u entries %s exhausted", LD_MAX_VECTORS,
			  FORMAT_BYTES (MAX_VECTORS * sizeof (unsigned)));
	  enlarged++;
	  kissat_inc_stack_enlarge (solver, (chars *) stack, sizeof (unsigned));
      ...
      if (enlarged)
	{
	  ...
	  kissat_inc_phase (solver, "vectors",
  ...
  unsigned *begin_old_vector = kissat_inc_begin_vector (vectors, vector);
  ...
  kissat_inc_add_usable (vectors, delta_size);
...
void
kissat_inc_defrag_vectors (kissat * solver, vectors * vectors,
		       unsigned size_unsorted, vector * unsorted)
{
  ...
  unsigned *sorted = kissat_inc_malloc (solver, bytes);
  ...
  kissat_inc_free (solver, sorted, bytes);
#ifndef QUIET
  const size_t freed = END_STACK (*stack) - p;
  double freed_fraction = kissat_inc_percent (freed, size_vectors);
  kissat_inc_phase (solver, "defrag", GET (defragmentations),
  ...
  kissat_inc_check_vectors (solver);
  STOP (defrag);
}

void
kissat_inc_remove_from_vector (kissat * solver,
			   vectors * vectors, vector * vector,
			   unsigned remove)
{
  unsigned *begin = kissat_inc_begin_vector (vectors, vector), *p = begin;
  const unsigned *end = kissat_inc_end_vector (vectors, vector);
  assert (p != end);
  while (*p != remove)
    p++, assert (p != end);
  while (++p != end)
...
void
kissat_inc_resize_vector (kissat * solver, vectors * vectors, vector * vector,
		      sector new_size)
{
  ...
  unsigned *begin = kissat_inc_begin_vector (vectors, vector);
  unsigned *end = begin + new_size;
  size_t delta = old_size - new_size;
  kissat_inc_add_usable (vectors, delta);
...
void
kissat_inc_release_vector (kissat * solver, vectors * vectors, vector * vector)
{
  kissat_inc_resize_vector (solver, vectors, vector, 0);
}

#ifdef CHECK_VECTORS

void
kissat_inc_check_vector (vectors * vectors, vector * vector)
{
  const unsigned *begin = kissat_inc_begin_vector (vectors, vector);
  const unsigned *end = kissat_inc_end_vector (vectors, vector);
...
void
kissat_inc_check_vectors (kissat * solver)
{
  for (all_literals (lit))
    {
      vector *vector = &WATCHES (lit);
      kissat_inc_check_vector (&solver->vectors, vector);
    }
