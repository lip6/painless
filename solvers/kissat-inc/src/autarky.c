...
determine_autarky (kissat * solver, value * autarky, unsigneds * work)
{
  ...
  if (assigned)
    kissat_inc_phase (solver, "autarky", GET (autarky_determined),
  ...
  for (all_clauses (c))
      ...
      else if (!c->garbage)
	{
	  ...
	  for (all_literals_in_clause (lit, c))
		...
		continue;
	      if (ref == INVALID_REF)
		ref = kissat_inc_reference_clause (solver, c);
	      kissat_inc_connect_literal (solver, lit, ref);
	    }
	}
    }
...
#ifndef QUIET
  if (flushed_large)
    kissat_inc_very_verbose (solver, "flushed %zu large clause references",
			 flushed_large);
  if (flushed_binaries)
    kissat_inc_very_verbose (solver, "flushed %zu autarky binary clauses",
			 flushed_binaries);
#else
...
autarky_literal (kissat * solver, unsigned lit)
{
  ...
  INC (autarky);
  kissat_inc_mark_eliminated_variable (solver, IDX (lit));
  word *arena = BEGIN_STACK (solver->arena);
...
void
kissat_inc_autarky (kissat * solver)
{
  ...
  value *autarky = kissat_inc_calloc (solver, LITS, sizeof (value));
  unsigneds work;
  INIT_STACK (work);
  ...
  kissat_inc_dealloc (solver, autarky, LITS, sizeof (value));
