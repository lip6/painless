  ...
  if (solver->stable)
    {
      ...
      kissat_inc_phase (solver, "stable", GET (stable_modes),
  ...
  else
    {
      ...
      const uint64_t delta = interval * (kissat_inc_quadratic (count) + 1);
      limits->mode.conflicts = CONFLICTS + delta;
      kissat_inc_phase (solver, "focus", GET (focused_modes),
...
{
#ifndef QUIET
  if (kissat_inc_verbosity (solver) < 2)
    return;

  const double current_time = kissat_inc_process_time ();
  ...
  // *INDENT-OFF*
  kissat_inc_very_verbose (solver, "%s mode took %.2f seconds "
...
switch_to_focused_mode (kissat * solver)
{
  ...
  kissat_inc_update_queue (solver, solver->links, last);
  kissat_inc_new_focused_restart_limit (solver);
}

void
kissat_inc_update_scores (kissat * solver)
{
  heap *scores = solver->heuristic==0?&solver->scores:&solver->scores_chb;
  for (all_variables (idx))
    if (ACTIVE (idx) && !kissat_inc_heap_contains (scores, idx))
      kissat_inc_push_heap (solver, scores, idx);
}
...
switch_to_stable_mode (kissat * solver)
{
  ...
  kissat_inc_update_scores (solver);
}

void
kissat_inc_switch_search_mode (kissat * solver)
{
