...
#ifndef QUIET
void kissat_inc_report_termination (kissat *, int bit, const char *file,
				long lineno, const char *fun);
#endif

static inline bool
kissat_inc_terminated (kissat * solver, int bit,
...
#endif
#ifndef QUIET
  kissat_inc_report_termination (solver, bit, file, lineno, fun);
#else
  (void) file;
  (void) lineno;
  (void) fun;
...
#define TERMINATED(BIT) \
  kissat_inc_terminated (solver, BIT, __FILE__, __LINE__, __func__)
