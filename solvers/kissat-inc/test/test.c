...
{
  const double start = kissat_inc_wall_clock_time ();
...
#ifndef QUIET
  kissat_inc_banner ("", "TISSAT Tester for KISSAT");
  tissat_line ();
...
do { \
  if ((tissat_found_ ## FLAG = kissat_inc_find_executable (#EXECUTABLE))) \
    { \
...
do { \
  if ((tissat_found_ ## EXECUTABLE = kissat_inc_find_executable (#EXECUTABLE))) \
    { \
  ...
  if (tissat_scheduled)
	...
	tissat_line ();
      tissat_message ("All %u test jobs %ssucceeded%s in %.2f seconds.",
		      tissat_scheduled,
		      kissat_inc_bold_green_color_code (1),
		      kissat_inc_normal_color_code (1),
		      kissat_inc_wall_clock_time () - start);
      tissat_release_jobs ();
    }
  else
