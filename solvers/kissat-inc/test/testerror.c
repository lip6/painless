      ...
      else
	{
	  ...
	  if (sig != SIGABRT)
	    FATAL ("child terminated by signal '%d' (%s) "
		   ...
		   sig, kissat_inc_signal_name (sig), (int) (SIGABRT));
	}
    }
  else
    {
      kissat_inc_reset_signal_handler ();
      kissat_inc_fatal ("real fatal error message triggering 'abort'");
      exit (0);
    }
}
...
test_fake_fatal_error (void)
{
  ...
  if (val)
    ...
    {
      kissat_inc_fatal ("faked fatal error message triggering long jump");
      kissat_inc_call_function_instead_of_abort (0);
...
{
  kissat_inc_call_function_instead_of_abort (just_return_call_back);
  kissat_inc_fatal ("faked just returning fatal error message");
}
