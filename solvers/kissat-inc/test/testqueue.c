...
#define size 16
  ...
  kissat_inc_init_queue (queue);
  for (int i = 0; i < size; i++)
    kissat_inc_enqueue (&solver, i);
  int c = 0;
  for (int i = queue->first; i >= 0; i = links[i].next, c++)
    assert (i == c);
  kissat_inc_move_to_front (&solver, queue->last);
  print_queue (queue, links);
  for (int i = 0; i < size; i += 2)
    kissat_inc_move_to_front (&solver, i);
  print_queue (queue, links);
  ...
  values[2 * search] = 1;
  values[2 * search + 1] = -1;
  kissat_inc_move_to_front (&solver, search);
  print_queue (queue, links);
  for (unsigned idx = 0; idx < size; idx++)
    {
      const unsigned lit = 2 * idx;
