...
add_unit_to_satisfy_redundant_clause (kissat * solver)
{
  if (solver->values[REDUNDANT_LITERAL])
    return;
  kissat_inc_assign_unit (solver, REDUNDANT_LITERAL);
  flush_unit (solver);
}
...
add_unit_to_satisfy_irredundant_clause (kissat * solver)
{
  if (solver->values[IRREDUNDANT_LITERAL])
    return;
  kissat_inc_assign_unit (solver, IRREDUNDANT_LITERAL);
  flush_unit (solver);
}
...
{
  int ilit;
  ilit = kissat_inc_import_literal (solver, 1);
  assert (ilit == 0);
  kissat_inc_activate_literal (solver, ilit);
  ilit = kissat_inc_import_literal (solver, 2);
  assert (ilit == 2);
  kissat_inc_activate_literal (solver, ilit);
  ilit = kissat_inc_import_literal (solver, 3);
  assert (ilit == 4);
  kissat_inc_activate_literal (solver, ilit);
  ilit = kissat_inc_import_literal (solver, 4);
  assert (ilit == 6);
  kissat_inc_activate_literal (solver, ilit);
}
...
add_large_redundant_clause (kissat * solver)
{
  ...
  kissat_inc_new_redundant_clause (solver, 2);
}
...
add_large_irredundant_clause (kissat * solver)
{
  ...
  PUSH_STACK (solver->clause.lits, 6);
  kissat_inc_new_irredundant_clause (solver);
}
...
test_collect (void)
{
  ...
  for (unsigned i = 0; i < (1u << bits); i++)
	...
	{
	  bool compact = !(i & (i << 8));
	  kissat_inc_sparse_collect (solver, compact, 0);
	}
      else if (i & (i << 8))
	{
	  if (!redundant_unit)
      ...
      if (i & (i << 9))
	{
	  ...
	  bool flush = !(i & (i << 10));
	  kissat_inc_resume_sparse_mode (solver, flush, 0, 0);
	}
      else if (i & (i << 10))
	{
	  if (!redundant_unit)
