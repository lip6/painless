...
test_format (void)
{
  ...
  (void) kissat_inc_next_format_string (&format);	// coverage ...

#define FORMAT(TYPE,EXPR,EXPECTED) \
do { \
  const char * RES = kissat_inc_format_ ## TYPE (&format, EXPR); \
  if (strcmp (RES, EXPECTED)) \
    FATAL ("kissat_inc_format_" #TYPE " (.., " #EXPR ") = \"%s\" but expected \"%s\"", RES, EXPECTED); \
  printf ("kissat_inc_format_" #TYPE " (.., " #EXPR ") = \"%s\" as expected\n", RES); \
} while (0)

#define FORMAT_signs(SIZE,SIGNS,EXPECTED) \
do { \
  const char * RES = kissat_inc_format_signs (&format, SIZE, SIGNS); \
  if (strcmp (RES, EXPECTED)) \
    FATAL ("kissat_inc_format_signs (.., " #SIZE ", " #SIGNS ") = \"%s\" but expected \"%s\"", RES, EXPECTED); \
  printf ("kissat_inc_format_signs (.., " #SIZE ", " #SIGNS ") = \"%s\" as expected\n", RES); \
} while (0)
...
do { \
  const char * res = kissat_inc_format_value (&format, BOOLEAN, EXPR); \
  if (strcmp (res, EXPECTED)) \
    FATAL ("kissat_inc_format_value (.., " #BOOLEAN ", " #EXPR ") = \"%s\" but expected \"%s\"", res, EXPECTED); \
  printf ("kissat_inc_format_value (.., " #BOOLEAN ", " #EXPR ") = \"%s\" as expected\n", res); \
} while (0)
