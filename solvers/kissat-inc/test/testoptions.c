...
do { \
  assert (kissat_inc_parse_option_value (STR, &value)); \
  if (value != VAL) \
...
do { \
  valstr = kissat_inc_parse_option_name (STR, NAME); \
  if (!valstr) \
    FATAL ("kissat_inc_parse_option_name (\"" STR "\", \"" NAME "\") returns 0"); \
  if (strcmp (valstr, EXPECTED)) \
    FATAL ("kissat_inc_parse_option_name (\"" STR "\", \"" NAME "\") != \"" EXPECTED "\""); \
...
do { \
  valstr = kissat_inc_parse_option_name (STR, NAME); \
  if (valstr) \
    FATAL ("kissat_inc_parse_option_name (\"" STR "\", \"" NAME "\") succeeded"); \
...
{
  options options;
  kissat_inc_init_options (&options);
  const opt *opt = kissat_inc_options_has ("eliminate");
  assert (opt);
  assert (opt->low == 0);
  assert (opt->high == 1);
  kissat_inc_options_set_opt (&options, opt, 0);
  assert (options.eliminate == 0);
  kissat_inc_options_set_opt (&options, opt, 1);
  assert (options.eliminate == 1);
  kissat_inc_options_set_opt (&options, opt, -1);
  assert (options.eliminate == 0);
  kissat_inc_options_set_opt (&options, opt, 2);
  assert (options.eliminate == 1);
  kissat_inc_options_set_opt (&options, opt, INT_MIN);
  assert (options.eliminate == 0);
  kissat_inc_options_set_opt (&options, opt, INT_MAX);
  assert (options.eliminate == 1);

  assert (!kissat_inc_options_get (&options, "nonexistingoption"));
}

static void
test_options_parse_arg (void)
{
  char buffer[kissat_inc_options_max_name_buffer_size];
  int value;

#define CHECKARG(STR,NAME,VAL) \
do { \
  assert (kissat_inc_options_parse_arg (STR, buffer, &value)); \
...
do { \
  ...
  //
  {
    char tmp[2 * kissat_inc_options_max_name_buffer_size];
