...
{
  assert (!kissat_inc_file_readable (0));
  assert (!kissat_inc_file_writable (0));
  file file;
  ...
  kissat_inc_close_file (&file);
  assert (!kissat_inc_open_to_write_file (&file, "/root/directory/not-writable"));
  assert (!kissat_inc_file_size ("/root/directory/not-writable"));
}
...
do { \
  bool RES = kissat_inc_file_readable (PATH); \
  if (RES && EXPECTED) \
...
do { \
  bool RES = kissat_inc_file_writable (PATH); \
  if (RES && EXPECTED) \
...
{
  const size_t expected_bytes = kissat_inc_file_size ("../test/file/0");
#define READ_COMPRESSED(EXPECTED,EXECUTABLE,PATH) \
do { \
  file file; \
  if (!kissat_inc_find_executable (EXECUTABLE)) \
    { \
      printf ("skipping '%s': could not find executable '%s'\n", \
              EXECUTABLE, PATH); \
      break; \
    } \
  bool res = kissat_inc_open_to_read_file (&file, PATH); \
  if (res && EXPECTED) \
  ...
  if (!res) \
    break; \
  int ch; \
  while ((ch = kissat_inc_getc (&file)) != EOF) \
    ; \
...
do { \
  if (!kissat_inc_find_executable (EXECUTABLE)) \
    ...
    { \
      ...
      if (!kissat_inc_open_to_write_file (&file, path)) \
	FATAL ("failed to write compressed '%s'", path); \
      else \
	{ \
	  kissat_inc_putc (&file, '4'); \
	  kissat_inc_putc (&file, '2'); \
	  kissat_inc_putc (&file, '\n'); \
	  kissat_inc_close_file (&file); \
	  printf ("reading single '42' line from compressed '%s'\n", path); \
	  if (!kissat_inc_open_to_read_file (&file, path)) \
	    FATAL ("failed to read compressed '%s'", path); \
	  int chars = 0; \
	  if ((++chars && kissat_inc_getc (&file) != '4') || \
	      (++chars && kissat_inc_getc (&file) != '2') || \
	      (++chars && kissat_inc_getc (&file) != '\n') || \
	      (++chars && kissat_inc_getc (&file) != EOF)) \
