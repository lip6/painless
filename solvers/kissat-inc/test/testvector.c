  ...
  for (unsigned i = 0; i < 100; i++)
    {
      if (!(i % (2 * N)))
	{
	  kissat_inc_defrag_vectors (solver, vectors, N, vector);
	  defrags++;
	}
      else
	{
	  ...
	  if (rand () % 3)
	    {
	      printf ("%u: push %u\n", i, j);
	      kissat_inc_push_vectors (solver, vectors, &vector[j], j);
	  ...
	  else if (vector[j].size)
	    {
	      ...
	      assert (count[j] > 0);
	      unsigned tmp =
		*kissat_inc_last_vector_pointer (vectors, &vector[j]);
	      kissat_inc_pop_vector (vectors, &vector[j]);
	      assert (tmp == j);
	      count[j]--;
	      popped++;
	    }
...
test_vector_fatal (void)
{
  ...
  if (val)
	...
	kissat_inc_enlarge_vector (solver, &vectors, &vector);
      }
      kissat_inc_call_function_instead_of_abort (0);
