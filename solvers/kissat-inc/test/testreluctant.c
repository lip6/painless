...
#define TICKS (1u<<LD_TICKS)
  ...
  kissat_inc_enable_reluctant (reluctant, PERIOD, LIMIT);
  assert (!kissat_inc_reluctant_triggered (reluctant));
  unsigned triggered = 0, last = 0;
  for (unsigned ticks = 1; ticks <= TICKS; ticks++)
    {
      kissat_inc_tick_reluctant (reluctant);
      if (!kissat_inc_reluctant_triggered (reluctant))
	continue;
      triggered++;
