...
void kissat_inc_update_scores (kissat *);
  ...
  for (unsigned i = 1; i <= 5; i++)
      ...
      while (prev < solver->scinc)
	{
	  ...
	  if (prev >= solver->scinc ||
	      ...
	      kissat_inc_get_heap_score (scores, 0) >= MAX_SCORE * 0.7 ||
	      kissat_inc_get_heap_score (scores, 1) >= MAX_SCORE * 0.7)
	    tissat_verbose ("%u.%u: score[0]=%g score[1]=%g scinc=%g",
			    i, count,
			    kissat_inc_get_heap_score (scores, 0),
			    kissat_inc_get_heap_score (scores, 1), solver->scinc);
	}
    }
