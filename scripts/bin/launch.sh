...
source $script_dir/../lib/myLibrary.sh
...
if [ $# -lt 3 ]; then
    echo "Usage: $0 <path_to_parameters_file> <path_to_topology_file> <path_to_input_file> "
    exit 1
fi
...
if [ ! -f "$topology_file" ]; then
    echo "Topology file $topology_file does not exist or is not a file"
    exit 1
fi
...
instance_name="${solver}_L${lstrat}_G${gstrat}${dist}-$(date +"%d_%m_%Y-%H_%M")_$4"
...
args="--topology=$topology_file -v=$verbose -t=$timeout" #-shr-strat=$lstrat -shr-sleep=$shr_sleep $flags
...
for f in $(cat ${input_files}); do
    ...
    "SATISFIABLE")
        ...
        ((nbSAT++))
        output=$("$script_dir/../analysis/SAT" "$f" "$output_file")
