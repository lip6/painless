	...
	enum
	{
		...
		CoeffReadCost =
			evaluator<T>::CoeffReadCost, // NOTE What if an evaluator evaluate itself into a temporary?
										 ...
										 //      evaluate, especially if n>1. This situation is already taken care by
										 //      the EvalBeforeNestingBit flag, which is turned ON for all evaluator
										 //      creating a temporary. This flag is then propagated by the parent
										 //      evaluators. Another solution could be to count the number of temps?
...
is_same_dense(const T1& mat1, const T2& mat2, typename enable_if<possibly_same_dense<T1, T2>::value>::type* = 0)
{
...
template<typename T1, typename T2>
EIGEN_DEVICE_FUNC bool
is_same_dense(const T1&, const T2&, typename enable_if<!possibly_same_dense<T1, T2>::value>::type* = 0)
{
	return false;
}
