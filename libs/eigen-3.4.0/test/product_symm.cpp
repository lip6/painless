...
void
symm(int size = Size, int othersize = OtherSize)
{
	...
	VERIFY_IS_APPROX(rhs12.noalias() +=
					 s1 * ((m2.adjoint()).template selfadjointView<Lower>() * (s2 * rhs3).conjugate()),
					 rhs13 += (s1 * m1.adjoint()) * (s2 * rhs3).conjugate());
	...
	// see bug 1741
	{
		...
		Map<Rhs2, 0, Stride<Dynamic, 2>> map2(
			buffer.data(), rhs22.rows(), rhs22.cols(), Stride<Dynamic, 2>(2 * rhs22.outerStride(), 2));
		buffer.setZero();
