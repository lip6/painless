...
struct generic_i0e<T, float>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		T y_gt_eight =
			pmul(internal::pchebevl<T, 7>::run(psub(pdiv(pset1<T>(32.0f), y), pset1<T>(2.0f)), B), prsqrt(y));
...
struct generic_k0<T, float>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		x_le_two = pmadd(generic_i0<T, float>::run(x), pnegate(plog(pmul(pset1<T>(0.5), x))), x_le_two);
		x_le_two = pselect(pcmp_le(x, pset1<T>(0.0)), MAXNUM, x_le_two);
		T x_gt_two = pmul(pmul(pexp(pnegate(x)), internal::pchebevl<T, 10>::run(psub(pdiv(pset1<T>(8.0), x), two), B)),
						  prsqrt(x));
		return pselect(pcmp_le(x, two), x_le_two, x_gt_two);
	}
};
...
struct generic_k0<T, double>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		x_le_two = pselect(pcmp_le(x, pset1<T>(0.0)), MAXNUM, x_le_two);
		T x_gt_two =
			pmul(pmul(pexp(-x), internal::pchebevl<T, 25>::run(psub(pdiv(pset1<T>(8.0), x), two), B)), prsqrt(x));
		return pselect(pcmp_le(x, two), x_le_two, x_gt_two);
	}
};
...
struct generic_k1<T, float>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		x_le_two = pmadd(generic_i1<T, float>::run(x), plog(pmul(pset1<T>(0.5), x)), x_le_two);
		x_le_two = pselect(pcmp_le(x, pset1<T>(0.0)), MAXNUM, x_le_two);
		T x_gt_two = pmul(pexp(pnegate(x)),
						  pmul(internal::pchebevl<T, 10>::run(psub(pdiv(pset1<T>(8.0), x), two), B), prsqrt(x)));
		return pselect(pcmp_le(x, two), x_le_two, x_gt_two);
	}
};
...
struct generic_k1<T, double>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		x_le_two = pselect(pcmp_le(x, pset1<T>(0.0)), MAXNUM, x_le_two);
		T x_gt_two =
			pmul(pexp(-x), pmul(internal::pchebevl<T, 25>::run(psub(pdiv(pset1<T>(8.0), x), two), B), prsqrt(x)));
		return pselect(pcmp_le(x, two), x_le_two, x_gt_two);
	}
};
...
struct generic_j0<T, float>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		const float JP[] = { -6.068350350393235E-008f,
		...
		T y = pabs(x);
		T z = pmul(y, y);
		T y_le_two = pselect(pcmp_lt(y, pset1<T>(1.0e-3f)),
...
struct generic_y0<T, float>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		const float YP[] = { 9.454583683980369E-008f,
...
struct generic_j1<T, float>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		const float JP[] = { -4.878788132172128E-009f,
...
struct generic_y1<T, float>
{
	...
	static EIGEN_STRONG_INLINE T run(const T& x)
	{
		...
		const float YP[] = { 8.061978323326852E-009f,
		...
		x_le_two =
			pmadd(x_le_two, x, pmul(TWOOPI, pmadd(generic_j1<T, float>::run(x), plog(x), pdiv(pset1<T>(-1.0f), x))));
