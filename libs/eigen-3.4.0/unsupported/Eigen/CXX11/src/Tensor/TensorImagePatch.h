...
struct ImagePatchPaddingOp
{
	...
	typedef typename Self::Scalar Scalar;
	typedef typename packet_traits<Scalar>::type Packet;
	static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void Run(const Index num_coeff_to_pad,
...
struct TensorEvaluator<const TensorImagePatchOp<Rows, Cols, ArgType>, Device>
{
		...
		if (op.padding_explicit()) {
			...
			switch (op.padding_type()) {
				case PADDING_VALID:
					...
					m_rowPaddingTop = numext::maxi<Index>(
						0, ((m_outputRows - 1) * m_row_strides + m_patch_rows_eff - m_input_rows_eff) / 2);
					m_colPaddingLeft = numext::maxi<Index>(
						0, ((m_outputCols - 1) * m_col_strides + m_patch_cols_eff - m_input_cols_eff) / 2);
					break;
				case PADDING_SAME:
	...
	EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorOpCost costPerCoeff(bool vectorized) const
	{
		...
		const double compute_cost = 3 * TensorOpCost::DivCost<Index>() + 6 * TensorOpCost::MulCost<Index>() +
