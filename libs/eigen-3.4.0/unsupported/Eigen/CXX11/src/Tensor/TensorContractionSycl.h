 ...
 */
template<bool PacketLoad,
		 bool is_coalesced_layout,
		 bool,
		 typename PacketType,
		 typename TensorMapper,
		 typename StorageIndex>
static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename ::Eigen::internal::enable_if<PacketLoad, PacketType>::type
read(const TensorMapper& tensorMapper, const StorageIndex& NCIndex, const StorageIndex& CIndex, const StorageIndex& ld)
{
...
static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename ::Eigen::internal::enable_if<!PacketLoad, PacketType>::type
read(const TensorMapper& tensorMapper, const StorageIndex& NCIndex, const StorageIndex& CIndex, const StorageIndex&)
{
...
static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename ::Eigen::internal::
	enable_if<Eigen::internal::unpacket_traits<PacketType>::size != 1 && dt == data_source::global_mem, void>::type
	write(PacketType& packet_data, DataScalar* ptr)
{
...
template<>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool
check_boundary<false>(bool cond)
{
	return cond;
}
...
struct ThreadProperties
{
	...
	EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ThreadProperties(const StorageIndex linearLocalThreadId_,
...
class TensorContractionKernel
{
  public:
	typedef
		typename Eigen::TensorSycl::internal::Vectorise<OutScalar, Eigen::SyclDevice, Vectorizable>::PacketReturnType
			PacketReturnType;
	...
	typedef BlockProperties<is_lhs_transposed,
							false,
							input_mapper_properties::is_lhs_matrix && Vectorizable,
							PacketReturnType>
		LHSBlockProperties;

	typedef BlockProperties<is_rhs_transposed,
							true,
							input_mapper_properties::is_rhs_matrix && Vectorizable,
							PacketReturnType>
		RHSBlockProperties;
	 ...
	 * the kernel as an accessor, the private memory can only allowed to be allocated statically. Since we are
	 abstracting
	 ...
	 *
	 * \tparam contraction_type: it is an enum value representing whether the local memroy/no local memory
	 implementation of the algorithm to be used
	...
	struct TiledMemory
	{
		...
		TiledMemory(const ThreadProperties<StorageIndex>&,
					local_ptr,
	...
	EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorContractionKernel(Scratch scratch_,
	...
	EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void operator()(cl::sycl::nd_item<1> itemID)
	{
		...
		auto thread_properties = ThreadProperties<StorageIndex>(linearLocalThreadId,
																kGroupId,
																mGroupOffset,
																nGroupOffset,
																kGroupOffset,
																	  ...
																	  PacketReturnType* privateRes)
	{
		...
		for (StorageIndex wLPTN = 0; wLPTN < Properties::WorkLoadPerThreadN; wLPTN++) {
			...
			for (StorageIndex wLPTM = 0; wLPTM < Properties::WorkLoadPerThreadM / PacketSize; wLPTM++) {
				...
				Eigen::TensorSycl::internal::PacketWrapper<PacketReturnType, PacketSize>::set_packet(
					lhsPack, lhs_block_ptr + lhs_index);
	...
	// memory. The store function is independent of local and no local concepts s that it can be abstract out in the
	// base class.
													 ...
													 StorageIndex nGlobalOffset)
	{
		...
		for (StorageIndex wLPTN = 0; wLPTN < Properties::WorkLoadPerThreadN / PrivateNStride; wLPTN++) {
			...
			// When local memory is used the PrivateNstride is always 1 because the coalesed access on N is loaded into
			// Local memory and extracting from local to global is the same as no transposed version. However, when
			// local memory is not used and RHS is transposed we packetize the load for RHS.
			EIGEN_UNROLL_LOOP
			for (StorageIndex nId = 0; nId < PrivateNStride; nId++) {
				...
				for (StorageIndex wLPTM = 0; wLPTM < Properties::WorkLoadPerThreadM / PacketSize; wLPTM++) {
					...
					if (check_boundary<is_internal_block>(chk_bound(globalRow, nId))) {
						// Store the final results in C. The C matrix has always M as a first StorageIndex and N as a
						// second StorageIndex Therefore it is always coalesced layout
						...
						for (StorageIndex mId = 0; mId < PacketSize; mId++) {
							...
							if (mOffset < triple_dim.M && (nGlobalOffset + nId < triple_dim.N)) {
								out_ptr[mOffset + outputLD] =
									Eigen::TensorSycl::internal::PacketWrapper<PacketReturnType, PacketSize>::scalarize(
										mId, privetOut);
							}
						}
					}
	...
	template<typename InputBlockProperties,
	...
	EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
		...
		extract_block(const Input& inpt,
		...
		for (StorageIndex cId = 0; cId < Properties::TileSizeDimK / InputBlockProperties::c_stride; cId++) {
			...
			for (StorageIndex ncId = 0; ncId < WorkLoadPerThreadNC / InputBlockProperties::nc_stride; ncId++) {
				if (check_boundary<is_internal_block>(chk_bound(cIndex, ncIndex))) {
					...
					write<StorageIndex,
						  (InputBlockProperties::is_coalesced_layout ? 1 : WorkLoadPerThreadNC),
					...
					for (StorageIndex i = 0; i < InputBlockProperties::elements_per_access; i++) {
						...
						OutScalar val = (ncInd < NC && cInd < triple_dim.K)
											? read<false,
												   InputBlockProperties::is_coalesced_layout,
												   InputBlockProperties::is_rhs,
												   OutScalar>(inpt, ncInd, cInd, ld)
											: OutScalar(0);
						write<StorageIndex,
							  (InputBlockProperties::is_coalesced_layout ? 1 : WorkLoadPerThreadNC),
							  data_source::private_mem>(
							val,
							private_ptr + (InputBlockProperties::is_coalesced_layout ? i : 0) +
								((InputBlockProperties::is_coalesced_layout ? 0 : i) * WorkLoadPerThreadNC));
					}
				}

				// if it is lhs we have to load it packetised when the packet size is > 1, because the output is
				// coalesced. So even if M is not accessed in a coalesced mode, we have to load packet_size number of m
				// per thread.
			...
			// the previous for loop ( private_ptr += (ncId * nc_stride)) has already moved ptr with one
			// WorkLoadPerThreadNC
																	  ...
																	  bool& db_offset)
	{
		...
		extract_block<RHSBlockProperties, is_internal_block>(
			rhs,
			tiled_input_block.rhs_scratch_extract.ptr + (db_offset * Properties::TileSizeDimK * LSDR),
			tiled_input_block.rhs_extract_index,
			contraction_tp == contraction_type::local ? thread_properties.nGroupOffset
		...
		extract_block<LHSBlockProperties, is_internal_block>(
			lhs,
			tiled_input_block.lhs_scratch_extract.ptr + (db_offset * LSDL * Properties::TileSizeDimK),
			tiled_input_block.lhs_extract_index,
			contraction_tp == contraction_type::local ? thread_properties.mGroupOffset
		...
		for (StorageIndex k = 0; k < Properties::TileSizeDimK; k++) {
			compute_block_per_tile(tiled_input_block.lhs_scratch_ptr_compute + lhs_offset,
								   tiled_input_block.rhs_scratch_ptr_compute + rhs_offset,
								   privateRes);
			lhs_offset += LSDL;
			rhs_offset += LSDR;
		}
															 ...
															 OutPtr out_ptr)
	{
		...
		store<is_internal_block,
			...
			out_ptr + thread_properties.nGlobalOffset * triple_dim.M,
			privateRes,
			thread_properties.mGlobalOffset,
			thread_properties.nGlobalOffset);
	}
...
struct GeneralVectorTensor
{
	...
	// Since the access layout for a vector can always be coalesced, when LHS is a vector, we pass false and false to
	// make sure that the !^ is true When RHS is a vector, we pass true and true to make sure that the !^ is true.
	...
	EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE GeneralVectorTensor(Scratch scratch_,
	...
	EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void operator()(cl::sycl::nd_item<1> itemID)
	{
		...
		is_internal ? compute_panel<true>(itemID,
										  vec,
										  mat,
										  local_output,
										  out_ptr,
	...
	template<bool is_internal_block, typename OutPtr>
	static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void compute_panel(const cl::sycl::nd_item<1>& itemID,
																	const VectorMapper& vec,
																	const TensorMapper& mat,
																	OutScalar* local_output,
																	OutPtr out_ptr,
#ifdef EIGEN_SYCL_LOCAL_MEM_UNSET_OR_ON
																	OutScalar* scratch_ptr,
																	...
																	StorageIndex outScratchIndex)
	{
		...
		extract_block<VecBlockProperties,
					  is_internal_block,
					  KFactor,
	...
	template<typename InputBlockProperties,
			 ...
			 typename Input,
			 typename Local>
	static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void extract_block(const Input& inpt,
																	Local* local_ptr,
																	const StorageIndex& linearLocalThreadId,
																	const StorageIndex& cOffset,
		...
		for (StorageIndex cId = 0; cId < CFactor / InputBlockProperties::c_stride; cId++) {
			if (check_boundary<is_internal_block>(cIndex + InputBlockProperties::c_stride - 1 < C)) {
				auto val = read<InputBlockProperties::packet_load,
								InputBlockProperties::is_coalesced_layout,
								InputBlockProperties::is_rhs,
								typename InputBlockProperties::OutType>(inpt, StorageIndex(0), cIndex, StorageIndex(1));
				write<StorageIndex, 1, data_source::local_mem>(val, local_ptr);
			} else {
				EIGEN_UNROLL_LOOP
				for (StorageIndex i = 0; i < InputBlockProperties::elements_per_access; i++) {
					OutScalar val = (cIndex + i < C)
										? read<false,
											   InputBlockProperties::is_coalesced_layout,
											   InputBlockProperties::is_rhs,
											   OutScalar>(inpt, StorageIndex(0), cIndex + i, StorageIndex(1))
										: OutScalar(0);
...
struct GeneralScalarContraction
{
	...
	EIGEN_DEVICE_FUNC
	GeneralScalarContraction(Scratch scratch_,
							 const LhsMapper lhs_,
							 const RhsMapper rhs_,
							 OutAccessor out_res_,
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>,
	...
	: public TensorContractionEvaluatorBase<
		  TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>,
						  Eigen::SyclDevice>>
{
	...
	typedef TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, Device>
		Self;
	...
	void EIGEN_ALWAYS_INLINE
	adjustTT(EvaluatorPointerType buffer, const LhsMapper& lhs, const RhsMapper& rhs, const TripleDim& triple_dim) const
	{
#ifdef EIGEN_SYCL_LOCAL_MEM_UNSET_OR_ON
		...
		if (!(device().has_local_memory())) {
			...
			launchTT<TensorSycl::internal::contraction_type::no_local,
					 skinny,
					 input_mapper_properties,
					 PanelParameters>(buffer, lhs, rhs, triple_dim);
		}
#endif
	}

	template<TensorSycl::internal::contraction_type ct,
			 bool skinny,
			 typename input_mapper_properties,
			 typename Properties,
			 typename LhsMapper,
			 typename RhsMapper>
	void launchTT(EvaluatorPointerType buffer,
				  const LhsMapper& lhs,
				  const RhsMapper& rhs,
				  const TripleDim& triple_dim) const
	{
		...
		const StorageIndex numTilesPerGroup =
			Eigen::TensorSycl::internal::roundUp(totalTilesK, groupSizeK) / groupSizeK;
		...
		const StorageIndex scratchSize =
			(ct == TensorSycl::internal::contraction_type::local)
				? ((Properties::DoubleBuffer + 1) * (Properties::TileSizeDimM + Properties::BC) *
		...
		if (groupSizeK == 1) {
			typedef TensorSycl::internal::TensorContractionKernel<CoeffReturnType,
																  LhsScalar,
																  RhsScalar,
			...
			device().template binary_kernel_launcher<CoeffReturnType, ContractKernelName>(lhs,
																						  rhs,
																						  tmp_global_accessor,
																						  thread_range,
																						  scratchSize,
			...
			auto op = Op();
			typedef TensorSycl::internal::SecondStepPartialReduction<CoeffReturnType,
																	 StorageIndex,
																	 EvaluatorPointerType,
																	 EvaluatorPointerType,
																	 Op>
			...
			device().template unary_kernel_launcher<CoeffReturnType, ReductionKernel>(
				tmp_global_accessor,
				buffer,
				cl::sycl::nd_range<1>(cl::sycl::range<1>(StorageIndex(Eigen::TensorSycl::internal::roundUp(
										  triple_dim.M * triple_dim.N, localRange))),
									  cl::sycl::range<1>(localRange)),
				StorageIndex(1),
				op,
				StorageIndex(triple_dim.M * triple_dim.N),
				groupSizeK);
									  ...
									  StorageIndex C) const
	{
		...
		auto thread_range = cl::sycl::nd_range<1>(cl::sycl::range<1>(globalRange), cl::sycl::range<1>(localRange));
		if (cNumGroups > 1) {
			typedef Eigen::TensorSycl::internal::GeneralVectorTensor<CoeffReturnType,
																	 EvaluatorPointerType,
																	 VectorMapper,
																	 TensorMapper,
																	 StorageIndex,
				...
				ContractKernelName;
			CoeffReturnType* temp_pointer = static_cast<CoeffReturnType*>(
				device().allocate_temp(nonContractDim * cNumGroups * sizeof(CoeffReturnType)));
			EvaluatorPointerType tmp_global_accessor = device().get(temp_pointer);
									  ...
									  StorageIndex K) const
	{
		...
		typedef Eigen::TensorSycl::internal::GeneralScalarContraction<CoeffReturnType,
																	  LhsScalar,
																	  RhsScalar,
																	  EvaluatorPointerType,
																	  LhsMapper,
		...
		if (num_work_group > 1) {
			...
			typedef Eigen::internal::SumReducer<CoeffReturnType> Op;
			typedef TensorSycl::internal::SecondStepFullReducer<CoeffReturnType,
																Op,
																EvaluatorPointerType,
																EvaluatorPointerType,
																StorageIndex,
			...
			device().template unary_kernel_launcher<CoeffReturnType, GenericRKernel>(
				tmp_global_accessor,
				buffer,
				cl::sycl::nd_range<1>(cl::sycl::range<1>(local_range), cl::sycl::range<1>(local_range)),
				local_range,
				Op());
