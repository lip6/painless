...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
test_sycl_contraction_m(const Device& sycl_device)
{
...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
test_sycl_contraction_k(const Device& sycl_device)
{
	for (IndexType k = 32; k < 256; k++) {
...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
test_sycl_contraction_n(const Device& sycl_device)
{
	for (IndexType k = 32; k < 256; k++) {
...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
test_sycl_contraction_sizes(const Device& sycl_device)
{
...
void static test_no_out_of_bounds(const Device& sycl_device, IndexType m_size, IndexType k_size, IndexType n_size)
{
	...
	for (IndexType i = 0; i < t_result.size(); i++) {
		if (static_cast<DataType>(std::fabs(static_cast<DataType>(t_result(i) - host_result_data[i]))) <
			error_threshold) {
			continue;
		}
		...
		if (std::isnan(host_result_data[i])) {
			std::cout << "M : " << m_size << ", N : " << n_size << ", K : " << k_size
					  << ", invalid read detected at IndexType " << i << ": " << t_result(i) << " vs "
					  << host_result_data[i] << std::endl;
		} else {
			std::cout << "M : " << m_size << ", N : " << n_size << ", K : " << k_size
					  << ", mismatch detected at IndexType " << i << ": " << t_result(i) << " vs "
					  << host_result_data[i] << std::endl;
		}
...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
test_scalar(const Device& sycl_device, IndexType m_size, IndexType k_size, IndexType n_size)
{
...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
contraction_batch(const Device& sycl_device,
				  IndexType m_size,
				  IndexType k_size,
				  IndexType n_size,
				  IndexType m_batch,
...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
contraction_rhs_transposed(const Device& sycl_device, IndexType m_size, IndexType k_size, IndexType n_size)
{
	typedef typename Tensor<DataType, 1, DataLayout, IndexType>::DimensionPair DimPair;
	static const DataType error_threshold = DataType(1e-4);
	Eigen::array<IndexType, 2> left_dims = { { m_size, k_size } };
...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
contraction_lhs_transposed(const Device& sycl_device, IndexType m_size, IndexType k_size, IndexType n_size)
{
	typedef typename Tensor<DataType, 1, DataLayout, IndexType>::DimensionPair DimPair;
	static const DataType error_threshold = DataType(1e-4);
...
template<int DataLayout, typename DataType, typename IndexType, typename Device>
void
contraction_both_transposed(const Device& sycl_device, IndexType m_size, IndexType k_size, IndexType n_size)
{
	typedef typename Tensor<DataType, 1, DataLayout, IndexType>::DimensionPair DimPair;
	static const DataType error_threshold = DataType(1e-4);
	Eigen::array<IndexType, 2> left_dims = { { k_size, m_size } };
