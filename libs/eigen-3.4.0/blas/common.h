...
inline bool
check_op(const char* op)
{
	return OP(*op) != 0xff;
}

inline bool
check_side(const char* side)
{
	return SIDE(*side) != 0xff;
}

inline bool
check_uplo(const char* uplo)
{
...
make_vector(T* data, int size)
{
	return Map<Matrix<T, Dynamic, 1>>(data, size);
}

template<typename T>
Map<const Matrix<T, Dynamic, 1>>
make_vector(const T* data, int size)
{
...
template<typename T>
T*
get_compact_vector(T* x, int n, int incx)
{
	if (incx == 1)
		return x;
...
template<typename T>
T*
copy_back(T* x_cpy, T* x, int n, int incx)
{
	if (x_cpy == x)
		return 0;
